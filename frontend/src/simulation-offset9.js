...
export class Offset9PrintingSimulation{
 constructor(root,template){
  ...
  this.rotors=[];this.nozzles=[];this.suckers=[];this.deliveryBars=[];this.sheets=[];this.stack=[];this.inkVisible=false;this.pathVisible=false;this.onUpdate=null;
  this.feederSuctionActive=false;this.coatingActive=false;this.airGuidanceActive=false;this.deliveryBrakeActive=false;
  root.traverse(o=>{if(o.isMesh&&o.userData.rotor)this.rotors.push(o);if(o.userData.airNozzle)this.nozzles.push(o);if(o.userData.reciprocator)this.suckers.push(o);if(o.userData.deliveryGripperBar)this.deliveryBars.push(o);});
 ...
 state(){
  ...
  return {available:true,blocked:false,active:this.active,running:this.running,paused:this.paused,speed:this.speed,stage:OFFSET9_SIMULATION_STAGES[index],completed:this.completed,progress,
   ...
   mechanismCount:this.rotors.length+this.suckers.length+this.nozzles.length+this.deliveryBars.length,inkFlowCount:0,uvLampCount:0,uvActive:false,pathVisible:this.pathVisible,inkFlowVisible:false,
   feederSuctionActive:this.feederSuctionActive,coatingActive:this.coatingActive,airGuidanceActive:this.airGuidanceActive,deliveryBrakeActive:this.deliveryBrakeActive,deliveryGripperActive:this.active&&this.running};
 }
 ...
 resetMechanisms(){
  ...
  this.feederSuctionActive=this.coatingActive=this.airGuidanceActive=this.deliveryBrakeActive=false;
 }
 spinRotors(dt){
 ...
 updateFeeder(){
  const cadence=12/this.sheets.length,phase=(this.elapsed%cadence)/cadence,pick=phase<.34;this.feederSuctionActive=pick;
 ...
 updateSheets(){
  ...
  this.coatingActive=coat;this.airGuidanceActive=air;this.deliveryBrakeActive=brake;
  for(const nozzle of this.nozzles){const world=new THREE.Vector3();nozzle.getWorldPosition(world);const active=this.sheets.some(s=>s.mesh.visible&&Math.abs(s.mesh.position.x-world.x)<.38);nozzle.material.emissive?.setHex(active?0x2fabc3:0);nozzle.material.emissiveIntensity=active?1.25:0;}
 }
