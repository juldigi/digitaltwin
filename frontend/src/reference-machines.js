...
export class ReferenceProcessSimulation{
 ...
 bindFolder(){
  ...
  this.folderBlank=g;
  this.folder={leftPivot,rightPivot,glueStrip,cartonBlankGeometryIsSchematic:true,installedCrashLockVerified:false,installedFourSixCornerVerified:false,glueApplicatorTypeVerified:false,foldingActive:false,glueZoneActive:false,compressionActive:false};
 }
 folderStatus(){
  const p=this.active?(this.elapsed%this.cycle)/this.cycle:0;
  let idx=0,folding=false,gluing=false,compressing=false;
  if(p<.14)idx=0;
  else if(p<.28)idx=1;
  else if(p<.48){idx=2;folding=true;}
  else if(p<.62){idx=3;gluing=true;}
  else if(p<.76){idx=4;folding=true;}
  else if(p<.90){idx=5;compressing=true;}
  else idx=6;
  return {p,idx,folding,gluing,compressing};
 }
 updateFolder(){
  if(!this.folder||!this.folderBlank)return;
  const s=this.folderStatus(),smooth=v=>{v=THREE.MathUtils.clamp(v,0,1);return v*v*(3-2*v);};
  this.folderBlank.visible=this.active;
  this.folderBlank.position.x=THREE.MathUtils.lerp(-5.20,5.22,smooth(s.p));
  let angle=0;
  if(s.p>=.28&&s.p<.48)angle=THREE.MathUtils.lerp(0,.62,smooth((s.p-.28)/.20));
  else if(s.p>=.48&&s.p<.62)angle=.62;
  else if(s.p>=.62&&s.p<.76)angle=THREE.MathUtils.lerp(.62,1.25,smooth((s.p-.62)/.14));
  else if(s.p>=.76)angle=1.25;
  this.folder.leftPivot.rotation.x=angle;
  this.folder.rightPivot.rotation.x=-angle;
  this.folder.glueStrip.visible=this.active&&s.p>=.48;
  this.folderBlank.position.y=s.compressing?.79:.82;
  this.folder.foldingActive=s.folding;this.folder.glueZoneActive=s.gluing;this.folder.compressionActive=s.compressing;
 }
 ...
 state(){
  const progress=this.active?(this.elapsed%this.cycle)/this.cycle:0,blankerState=this.family==='blanker'?this.blankerStatus():null,collatorState=this.family==='collator'?this.collatorStatus():null,imagesetterState=this.family==='imagesetter'?this.imagesetterStatus():null,zundState=this.family==='zund'?this.zundStatus():null,compressorState=this.family==='compressor'?this.compressorStatus():null,ahuState=this.family==='ahu'?this.ahuStatus():null,folderState=this.family==='folder'?this.folderStatus():null,idx=blankerState?.idx??collatorState?.idx??imagesetterState?.idx??zundState?.idx??compressorState?.idx??ahuState?.idx??folderState?.idx??this.stageIndex();
 ...
 start(){if(this.blocked){this.active=false;this.running=false;this.paused=false;this.onUpdate?.(this.state());return this.state();}this.active=true;this.running=true;this.paused=false;this.elapsed=0;this.lastNow=null;this.completed=0;if(this.processPiece)this.processPiece.visible=true;for(const s of this.collatorSheets)s.mesh.visible=true;if(this.imagesetterMedia)this.imagesetterMedia.visible=true;if(this.zundMaterial)this.zundMaterial.visible=true;for(const p of this.compressorParticles)p.mesh.visible=true;for(const p of this.ahuParticles)p.mesh.visible=true;if(this.folderBlank)this.folderBlank.visible=true;this.resetMotion();this.onUpdate?.(this.state());return this.state();}
 ...
 stop(){this.active=false;this.running=false;this.paused=false;this.elapsed=0;this.lastNow=null;this.completed=0;this.resetMotion();if(this.processPiece)this.processPiece.visible=false;for(const s of this.collatorSheets)s.mesh.visible=false;if(this.imagesetterMedia)this.imagesetterMedia.visible=false;if(this.zundMaterial)this.zundMaterial.visible=false;for(const p of this.compressorParticles)p.mesh.visible=false;for(const p of this.ahuParticles)p.mesh.visible=false;if(this.folderBlank)this.folderBlank.visible=false;this.onUpdate?.(this.state());return this.state();}
