...
export class ReferenceProcessSimulation{
 ...
 start(){if(this.blocked){this.active=false;this.running=false;this.paused=false;this.onUpdate?.(this.state());return this.state();}this.active=true;this.running=true;this.paused=false;this.elapsed=0;this.lastNow=null;this.completed=0;if(this.processPiece)this.processPiece.visible=true;for(const s of this.collatorSheets)s.mesh.visible=true;if(this.imagesetterMedia)this.imagesetterMedia.visible=true;if(this.zundMaterial)this.zundMaterial.visible=true;for(const p of this.ahuParticles)p.mesh.visible=true;if(this.folderBlank)this.folderBlank.visible=true;this.resetMotion();this.onUpdate?.(this.state());return this.state();}
