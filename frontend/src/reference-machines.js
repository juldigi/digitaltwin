...
export class ReferenceMachineTemplate extends UniversalMachineTemplate{
 ...
 enrichCollator(){
  ...
  this.root.userData.detailPass='V138_COLLATOR_SUCTION_AND_SEPARATION_AIR_CONTROL_CHAIN';
  this.root.userData.exactCollatorOemVerified=false;
  this.root.userData.exactCollatorModelVerified=false;this.root.userData.collatorAirArchitecture={installedTopologyVerified:false,suctionBlower:'FAMILY_REFERENCE',separationAir:'FAMILY_REFERENCE',individualBinAirAdjustment:'DOCUMENTED_ON_HORIZON_CLOSE_FAMILY_NOT_BMJ_CONFIRMED'};
  ...
  if(tower){
   ...
   for(let b=0;b<10;b++){
    ...
    if(air){
     ...
     const adjust=this.cyl(air,.020,.040,[-.06,y+.075,.42],'accent','z');this.tag(adjust,'collator-bin-separation-air-adjuster-reference','HORIZON_INDIVIDUAL_BIN_AIR_REFERENCE');adjust.userData.binIndex=b;
    }
   }
  }
  if(vac){
   const blower=this.findNode('collator-vacuum-blower');
   if(blower){const fan=this.motion(this.cyl(blower,.15,.28,[0,.42,.36],'dark','x'),'spin','x',11,.01,0,null);this.tag(fan,'vacuum-blower','MULTI_VENDOR_COLLATOR_FAMILY');const motor=this.cyl(blower,.10,.22,[-.23,.42,.36],'dark','x');this.tag(motor,'collator-suction-blower-motor-reference','HORIZON_SUCTION_AIR_REFERENCE');const gauge=this.cyl(blower,.055,.025,[.18,.58,.36],'glass','z');this.tag(gauge,'collator-vacuum-gauge-reference','SUCTION_SYSTEM_REFERENCE');}
   const manifold=this.findNode('collator-vacuum-manifold');
   if(manifold){this.tag(this.cyl(manifold,.030,1.30,[.04,.98,.44],'steel','y'),'vacuum-main-manifold','MULTI_VENDOR_COLLATOR_FAMILY');for(let b=0;b<10;b++){const y=.34+b*.145;const branch=this.cyl(manifold,.010,.42,[.04,y,.22],'steel','z');this.tag(branch,'vacuum-bin-branch','MULTI_VENDOR_COLLATOR_FAMILY');branch.userData.binIndex=b;const valve=this.cyl(manifold,.022,.045,[.04,y,.39],'accent','z');this.tag(valve,'collator-bin-suction-adjuster-reference','CROSS_FAMILY_CONTROL_REFERENCE');valve.userData.binIndex=b;}}
  }
  if(sense){
 ...
 enrichZund(){
  ...
  this.root.userData.detailPass='V138_ZUND_VACUUM_CONTROL_AND_MODULE_Z_CHAIN';
  ...
  this.root.userData.vacuumControlReference={installedTopologyVerified:false,s3SteplessWidthControl:'FAMILY_CAPABILITY_REFERENCE',g3S3VacuumHoldDown:true,zoneCountNotInferred:true};
  ...
  if(table){
   ...
   if(zones){
    ...
    const manifold=this.cyl(zones,.035,3.70,[0,.40,1.05],'steel','x');this.tag(manifold,'zund-vacuum-distribution-manifold-reference','ZUND_VACUUM_FUNCTIONAL_REFERENCE');
    for(const x of [-1.45,-.48,.48,1.45]){const branch=this.cyl(zones,.014,.40,[x,.47,.86],'steel','z');this.tag(branch,'zund-vacuum-zone-branch-reference','ZUND_VACUUM_FUNCTIONAL_REFERENCE');}
   }
  }
  ...
  if(ctl){
   ...
   if(vac){this.tag(this.box(vac,[.42,.42,.35],[-2.10,.38,-1.30],'dark',.02),'vacuum-generator-interface','ZUND_FAMILY_REFERENCE');this.tag(this.cyl(vac,.035,.48,[-1.95,.52,-1.08],'steel','y'),'vacuum-supply-duct-reference','ZUND_FAMILY_REFERENCE');const valve=this.box(vac,[.11,.16,.12],[-1.87,.72,-1.08],'accent',.006);this.tag(valve,'zund-vacuum-control-valve-reference','ZUND_VACUUM_FUNCTIONAL_REFERENCE');const sensor=this.cyl(vac,.055,.025,[-1.74,.72,-1.08],'glass','z');this.tag(sensor,'zund-vacuum-pressure-sensor-reference','ZUND_VACUUM_FUNCTIONAL_REFERENCE');const silencer=this.cyl(vac,.07,.20,[-2.30,.39,-1.30],'dark','x');this.tag(silencer,'zund-vacuum-exhaust-silencer-reference','VACUUM_SERVICE_REFERENCE');}
...
export class ReferenceProcessSimulation{
 ...
 bindBlanker(){
  ...
  this.blanker={platform,ram,separation:find('qf100-separation-interface'),receiving:find('qf100-receiving-tray'),
   xAxis:find('qf100-x-axis'),yAxis:find('qf100-y-axis'),hydraulicUnit:find('qf100-hydraulic-unit'),
   platformRest:platform?.position.clone()||new THREE.Vector3(),ramRest:ram?.position.clone()||new THREE.Vector3(),
   xServoActive:false,yServoActive:false,hydraulicPumpActive:false,hydraulicValvePressActive:false,hydraulicValveReturnActive:false,positionFeedbackActive:false};
 ...
 blankerStatus(){
  ...
  let idx=0,indexing=false,xIndexing=false,yIndexing=false,pressing=false,loading=false,separating=false,returning=false,platformAtPress=false,hydraulicPressure=false,hydraulicPump=false,valvePress=false,valveReturn=false,positionFeedback=false;
  if(p<.15){idx=0;loading=true;}
  else if(p<.32){idx=1;indexing=true;xIndexing=true;positionFeedback=true;}
  else if(p<.48){idx=2;indexing=true;yIndexing=true;positionFeedback=true;}
  else if(p<.58){idx=2;platformAtPress=true;positionFeedback=true;hydraulicPump=true;}
  else if(p<.72){idx=3;pressing=true;platformAtPress=true;hydraulicPressure=true;hydraulicPump=true;valvePress=true;}
  else if(p<.84){idx=4;platformAtPress=true;separating=true;hydraulicPump=true;valveReturn=true;}
  else {idx=6;indexing=true;xIndexing=true;returning=true;positionFeedback=true;}
  return {p,idx,indexing,xIndexing,yIndexing,pressing,loading,separating,returning,platformAtPress,hydraulicPressure,hydraulicPump,valvePress,valveReturn,positionFeedback,interlockSafe:!(indexing&&pressing)};
 }
 updateBlanker(){
  if(!this.blanker)return;
  const {p,indexing,xIndexing,yIndexing,pressing,loading,separating,hydraulicPump,valvePress,valveReturn,positionFeedback}=this.blankerStatus(),b=this.blanker;
  ...
  const spin=this.elapsed*10;
  for(const m of this.template.activeMeshes){
   if(!['x-axis-servo-motor','y-axis-servo-motor','hydraulic-pump-motor-reference'].includes(m.userData.mechanismRole))continue;
   ...
   const role=m.userData.mechanismRole,on=role==='x-axis-servo-motor'?xIndexing:role==='y-axis-servo-motor'?yIndexing:hydraulicPump;
   if(on){const axis=role==='y-axis-servo-motor'?AXIS.z:AXIS.x;m.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(axis,spin));}
  }
  b.xServoActive=xIndexing;b.yServoActive=yIndexing;b.hydraulicPumpActive=hydraulicPump;b.hydraulicValvePressActive=valvePress;b.hydraulicValveReturnActive=valveReturn;b.positionFeedbackActive=positionFeedback;
 }
 bindCollator(){
  const binCount=this.template.root.userData.modeledReferenceBinCount||10;
  const rotors=[],sensors=[];
  this.template.root.traverse(o=>{if(o.userData?.mechanismRole==='suction-rotor')rotors.push(o);if(['double-feed-ir-emitter-reference','double-feed-ir-receiver-reference'].includes(o.userData?.mechanismRole))sensors.push(o);});
  rotors.forEach(r=>{if(!r.userData.collatorRestQuaternion)r.userData.collatorRestQuaternion=r.quaternion.clone();});
  this.collator={binCount,rotors,sensors,installedBinCountVerified:this.template.root.userData.installedBinCountVerified===true,activeFeedBins:[],doubleFeedCheckActive:false,gatherTransportActive:false,airSeparationActive:false,rotorPickupActive:false,suctionBlowerActive:false,separationAirControlActive:false};
 ...
 updateCollator(){
  ...
  for(const item of this.collatorSheets){
   ...
   if(q<.30){
    activeBins.push(b);
    const t=smooth01(q/.30),lift=Math.sin(Math.PI*t)*.055;mesh.position.set(THREE.MathUtils.lerp(item.start.x,.48,t),item.start.y+lift,0);mesh.rotation.z=-.12*Math.sin(Math.PI*t);
   }else if(q<.72){
    mesh.rotation.z=0;
    const t=smooth01((q-.30)/.42);mesh.position.set(.58,THREE.MathUtils.lerp(item.start.y,.58,t),0);
   }else{
    mesh.rotation.z=0;const t=smooth01((q-.72)/.28);mesh.position.set(THREE.MathUtils.lerp(.58,1.08,t),.58+b*.010,0);
   }
  }
  ...
  this.collator.doubleFeedCheckActive=activeFeedCount>0;this.collator.gatherTransportActive=activeFeedCount>0||completedInSet>0;this.collator.airSeparationActive=activeBins.length>0;this.collator.rotorPickupActive=activeBins.length>0;this.collator.suctionBlowerActive=this.active;this.collator.separationAirControlActive=activeBins.length>0;
 }
 bindCTP(){
  ...
  this.ctpLaserBeamGeometry=new THREE.BoxGeometry(.012,.14,.012);this.ctpLaserBeamMaterial=new THREE.MeshStandardMaterial({color:0xb82323,emissive:0x8a1515,emissiveIntensity:1.1,roughness:.2,transparent:true,opacity:.88});
  this.ctpLaserBeam=new THREE.Mesh(this.ctpLaserBeamGeometry,this.ctpLaserBeamMaterial);this.ctpLaserBeam.name='SUPRASETTER-LASER-BEAM-REFERENCE';this.ctpLaserBeam.visible=false;this.ctpLaserBeam.position.set(0,1.18,0);this.root.add(this.ctpLaserBeam);
  this.ctpLaserSpotGeometry=new THREE.SphereGeometry(.020,10,8);this.ctpLaserSpot=new THREE.Mesh(this.ctpLaserSpotGeometry,this.ctpLaserBeamMaterial);this.ctpLaserSpot.name='SUPRASETTER-LASER-SPOT-REFERENCE';this.ctpLaserSpot.visible=false;this.ctpLaserSpot.position.set(0,1.155,0);this.root.add(this.ctpLaserSpot);
 }
 ctpStatus(){
  const p=this.active?(this.elapsed%this.cycle)/this.cycle:0;
 ...
 bindZund(){
  ...
  this.zund={
   ...
   vacuumHoldActive:false,vacuumControlActive:false,vacuumMode:'RELEASE',axisPathType:'SERPENTINE_REFERENCE_ONLY',toolActionActive:false,
 ...
 updateZund(){
  ...
  if(this.zundMaterial){this.zundMaterial.visible=this.active;this.zundMaterial.position.y=s.vacuumHold?.655:.69;}
  z.vacuumHoldActive=s.vacuumHold;z.vacuumControlActive=s.vacuumHold;z.vacuumMode=s.vacuumHold?'HOLD':'RELEASE';z.vacuumContactActive=s.vacuumHold;z.toolActionActive=s.axisMotion&&z.installedToolPackageVerified===true;z.toolClearanceMaintained=!z.toolActionActive;
 }
 bindCompressor(){
 ...
 updateCompressor(){
  ...
  for(const item of this.compressorParticles){
   ...
   m.visible=this.active&&(item.kind!=='distribution'||this.pathVisible);
  }
 ...
 updateAHU(){
  ...
  for(const item of this.ahuParticles){
   ...
   m.visible=this.active&&(item.kind!=='return'||this.pathVisible);
  }
 ...
 bindFolder(){
  ...
  this.folderBlank=g;
  const drives={feeder:null,fold:null,compression:null,delivery:null};this.template.root.traverse(o=>{const r=o.userData?.mechanismRole;if(r==='fgm-feeder-section-motor-reference')drives.feeder=o;else if(r==='fgm-primary-fold-section-motor-reference')drives.fold=o;else if(r==='fgm-compression-section-motor-reference')drives.compression=o;else if(r==='fgm-delivery-section-motor-reference')drives.delivery=o;});
  for(const m of Object.values(drives))if(m)m.userData.folderDriveRestQuaternion=m.quaternion.clone();
  this.folder={leftPivot,rightPivot,glueStrip,drives,cartonBlankGeometryIsSchematic:true,installedCrashLockVerified:false,installedFourSixCornerVerified:false,glueApplicatorTypeVerified:false,foldingActive:false,prebreakActive:false,alignmentActive:false,glueZoneActive:false,finalFoldActive:false,compressionActive:false,deliveryActive:false,feederDriveActive:false,foldDriveActive:false,compressionDriveActive:false,deliveryDriveActive:false,zonePositions:[-5.20,-3.45,-1.75,0,1.75,3.55,5.00,5.45]};
 }
 folderStatus(){
  const p=this.active?(this.elapsed%this.cycle)/this.cycle:0;
 ...
 updateFolder(){
  ...
  this.folderBlank.position.x=x;const alignT=s.p<.14?0:s.p<.29?smooth((s.p-.14)/.15):1;this.folderBlank.position.z=s.p<.29?THREE.MathUtils.lerp(.075,0,alignT):0;this.folderBlank.rotation.y=s.p<.29?THREE.MathUtils.lerp(.07,0,alignT):0;
  let angle=0;
  ...
  for(const [key,m] of Object.entries(f.drives||{})){if(!m)continue;m.quaternion.copy(m.userData.folderDriveRestQuaternion||new THREE.Quaternion());if(driveOn[key])m.quaternion.multiply(new THREE.Quaternion().setFromAxisAngle(AXIS.x,this.elapsed*8));}
 }
 ...
 state(){
  ...
  return {available:!this.blocked,blocked:this.blocked,blockedReason:this.blockedReason,referenceModel:true,evidenceGrade:this.template.cfg.evidence.grade,geometryStatus:this.template.cfg.evidence.geometry,
   ...
   oscillatorCount:this.motions.filter(x=>x.motion.type!=='spin').length,mechanismCount:this.family==='blanker'?this.template.activeMeshes.length:this.family==='zund'?2:this.motions.length,inkFlowCount:0,uvLampCount:0,uvActive:false,pathVisible:this.pathVisible,inkFlowVisible:this.inkFlowVisible,
   platformIndexing:blankerState?.indexing??false,blankerXServoActive:this.family==='blanker'?(this.blanker?.xServoActive??false):false,blankerYServoActive:this.family==='blanker'?(this.blanker?.yServoActive??false):false,blankerHydraulicPumpActive:this.family==='blanker'?(this.blanker?.hydraulicPumpActive??false):false,blankerHydraulicValvePressActive:this.family==='blanker'?(this.blanker?.hydraulicValvePressActive??false):false,blankerHydraulicValveReturnActive:this.family==='blanker'?(this.blanker?.hydraulicValveReturnActive??false):false,blankerPositionFeedbackActive:this.family==='blanker'?(this.blanker?.positionFeedbackActive??false):false,blankerLoading:blankerState?.loading??false,blankerPlatformAtPress:blankerState?.platformAtPress??false,blankerHydraulicPressureActive:blankerState?.hydraulicPressure??false,blankerSeparationActive:blankerState?.separating??false,blankerReturning:blankerState?.returning??false,blankingHeadPressing:blankerState?.pressing??false,mechanicalInterlockSafe:blankerState?.interlockSafe??true,
   modeledBinCount:collatorState?.modeledBinCount??null,installedBinCountVerified:collatorState?.installedBinCountVerified??null,activeBinFeeds:this.collator?.activeFeedCount??0,activeFeedBinIndexes:this.collator?.activeFeedBins??[],collatorSuctionBlowerActive:this.family==='collator'?(this.collator?.suctionBlowerActive??false):false,collatorSeparationAirControlActive:this.family==='collator'?(this.collator?.separationAirControlActive??false):false,airSeparationActive:this.family==='collator'?(this.collator?.airSeparationActive??false):false,rotorPickupActive:this.family==='collator'?(this.collator?.rotorPickupActive??false):false,doubleFeedCheckActive:this.family==='collator'?(this.collator?.doubleFeedCheckActive??false):false,gatherTransportActive:this.family==='collator'?(this.collator?.gatherTransportActive??false):false,completedSheetsInSet:this.collator?.completedSheetsInSet??0,
   exposureActive:imagesetterState?.exposure??false,cuttingActive:imagesetterState?.cutting??false,imagesetterMediaPresent:this.family==='imagesetter'?(this.imagesetter?.mediaPresent??false):false,imagesetterSupplyBrakeActive:this.family==='imagesetter'?(this.imagesetter?.supplyBrakeActive??false):false,imagesetterTensionValid:this.family==='imagesetter'?(this.imagesetter?.tensionValid??false):false,imagesetterCapstanEncoderActive:this.family==='imagesetter'?(this.imagesetter?.capstanEncoderActive??false):false,imagesetterPolygonAtSpeed:this.family==='imagesetter'?(this.imagesetter?.polygonAtSpeed??false):false,imagesetterExposurePermit:this.family==='imagesetter'?(this.imagesetter?.exposurePermit??false):false,imagesetterExposureComplete:this.family==='imagesetter'?(this.imagesetter?.exposureComplete??false):false,imagesetterCutterPermit:this.family==='imagesetter'?(this.imagesetter?.cutterPermit??false):false,imagesetterCutterHomeConfirmed:this.family==='imagesetter'?(this.imagesetter?.cutterHomeConfirmed??true):true,imagesetterOutputDetected:this.family==='imagesetter'?(this.imagesetter?.outputDetected??false):false,imagesetterInterlockSafe:this.family==='imagesetter'?(this.imagesetter?.interlockSafe??true):true,capstanAdvanceActive:this.family==='imagesetter'?(this.imagesetter?.capstanAdvanceActive??false):false,tensionRegulationActive:this.family==='imagesetter'?(this.imagesetter?.tensionRegulationActive??false):false,outputBoundaryActive:this.family==='imagesetter'?(this.imagesetter?.outputBoundaryActive??false):false,imagesetterMediaContactStage:this.family==='imagesetter'?(this.imagesetter?.mediaContactStage??null):null,punchInstalledVerified:this.imagesetter?.punch?.userData.installedOptionVerified===true,processorInstalledVerified:this.imagesetter?.processor?.userData.installedOptionVerified===true,exactScreenModelVerified:this.family==='imagesetter'?this.template.root.userData.exactScreenModelVerified:null,
   vacuumHoldActive:zundState?.vacuumHold??false,zundAxisMotionActive:zundState?.axisMotion??false,zundAxisPathType:this.family==='zund'?(this.zund?.axisPathType??null):null,zundToolActionActive:this.family==='zund'?(this.zund?.toolActionActive??false):false,zundVacuumControlActive:this.family==='zund'?(this.zund?.vacuumControlActive??false):false,zundVacuumMode:this.family==='zund'?(this.zund?.vacuumMode??null):null,zundVacuumContactActive:this.family==='zund'?(this.zund?.vacuumContactActive??false):false,zundToolClearanceMaintained:this.family==='zund'?(this.zund?.toolClearanceMaintained??true):null,ctpPlateLoaded:this.family==='ctp'?(this.ctp?.plateLoaded??false):false,ctpPlatePresent:this.family==='ctp'?(this.ctp?.platePresent??false):false,ctpRegisterConfirmed:this.family==='ctp'?(this.ctp?.registerConfirmed??false):false,ctpDrumAtLoadPosition:this.family==='ctp'?(this.ctp?.drumAtLoadPosition??false):false,ctpPlateClamped:this.family==='ctp'?(this.ctp?.plateClamped??false):false,ctpClampConfirmed:this.family==='ctp'?(this.ctp?.clampConfirmed??false):false,ctpTransportDriveActive:this.family==='ctp'?(this.ctp?.transportDriveActive??false):false,ctpDrumDriveActive:this.family==='ctp'?(this.ctp?.drumDriveActive??false):false,ctpDrumEncoderSync:this.family==='ctp'?(this.ctp?.drumEncoderSync??false):false,ctpExposurePermit:this.family==='ctp'?(this.ctp?.exposurePermit??false):false,ctpExposureActive:this.family==='ctp'?(this.ctp?.exposureActive??false):false,ctpLaserTraverseActive:this.family==='ctp'?(this.ctp?.laserTraverseActive??false):false,ctpUnloadPermit:this.family==='ctp'?(this.ctp?.unloadPermit??false):false,ctpOutputDetected:this.family==='ctp'?(this.ctp?.outputDetected??false):false,ctpInterlockSafe:this.family==='ctp'?(this.ctp?.interlockSafe??true):true,ctpLaserBeamVisible:this.family==='ctp'?(this.ctp?.laserBeamVisible??false):false,ctpMaterialContact:this.family==='ctp'?(this.ctp?.materialContact??null):null,ctpPunchInstalledVerified:this.family==='ctp'?(this.ctp?.punchInstalledVerified??false):null,installedToolPackageVerified:this.family==='zund'?(this.zund?.installedToolPackageVerified??false):null,toolActionEnabled:this.family==='zund'?(this.zund?.installedToolPackageVerified===true):null,registrationCameraInstalledVerified:this.family==='zund'?(this.zund?.installedIccVerified??false):null,toolInitializationInstalledVerified:this.family==='zund'?(this.zund?.installedItiVerified??false):null,
 ...
 setPathVisible(v){
  ...
  for(const item of this.compressorParticles)if(item.kind==='distribution')item.mesh.visible=this.active&&this.pathVisible;
  for(const item of this.ahuParticles)if(item.kind==='return')item.mesh.visible=this.active&&this.pathVisible;
  return this.state();
 }
 setInkFlowVisible(){this.inkFlowVisible=false;return this.state();}
 resetMotion(){for(const item of this.motions){item.mesh.position.copy(item.position);item.mesh.quaternion.copy(item.quaternion);}if(this.blanker){if(this.blanker.platform)this.blanker.platform.position.copy(this.blanker.platformRest);if(this.blanker.ram)this.blanker.ram.position.copy(this.blanker.ramRest);this.blanker.xServoActive=false;this.blanker.yServoActive=false;this.blanker.hydraulicPumpActive=false;this.blanker.hydraulicValvePressActive=false;this.blanker.hydraulicValveReturnActive=false;this.blanker.positionFeedbackActive=false;}if(this.processPiece){if(this.family==='blanker'&&this.processPiece.userData.blankerRest)this.processPiece.position.copy(this.processPiece.userData.blankerRest);else this.processPiece.position.x=this.processPiece.userData.startX;this.processPiece.visible=this.active;}for(const s of this.collatorSheets){s.mesh.position.copy(s.start);s.mesh.rotation.set(0,0,0);s.mesh.visible=this.active;}if(this.collator){this.collator.activeFeedCount=0;this.collator.activeFeedBins=[];this.collator.doubleFeedCheckActive=false;this.collator.gatherTransportActive=false;this.collator.airSeparationActive=false;this.collator.rotorPickupActive=false;this.collator.suctionBlowerActive=false;this.collator.separationAirControlActive=false;this.collator.completedSheetsInSet=0;for(const rotor of this.collator.rotors)rotor.quaternion.copy(rotor.userData.collatorRestQuaternion||new THREE.Quaternion());}if(this.imagesetterMedia){this.imagesetterMedia.position.set(-.82,.68,0);this.imagesetterMedia.visible=this.active;}if(this.ctpFlatPlate){this.ctpFlatPlate.position.set(-1.10,.68,0);this.ctpFlatPlate.rotation.set(0,0,0);this.ctpFlatPlate.visible=this.active;}if(this.ctpWrappedGroup)this.ctpWrappedGroup.rotation.set(0,0,0);if(this.ctpWrappedPlate)this.ctpWrappedPlate.visible=false;if(this.ctpLaserBeam)this.ctpLaserBeam.visible=false;if(this.ctpLaserSpot)this.ctpLaserSpot.visible=false;if(this.ctp){this.ctp.plateLoaded=false;this.ctp.platePresent=false;this.ctp.registerConfirmed=false;this.ctp.drumAtLoadPosition=false;this.ctp.plateClamped=false;this.ctp.clampConfirmed=false;this.ctp.transportDriveActive=false;this.ctp.drumDriveActive=false;this.ctp.drumEncoderSync=false;this.ctp.exposurePermit=false;this.ctp.exposureActive=false;this.ctp.laserTraverseActive=false;this.ctp.unloadPermit=false;this.ctp.outputDetected=false;this.ctp.interlockSafe=true;this.ctp.laserBeamVisible=false;this.ctp.materialContact='NONE';if(this.ctp.laser?.material?.emissive){this.ctp.laser.material.emissive.setHex(0);this.ctp.laser.material.emissiveIntensity=0;}}if(this.imagesetter){this.imagesetter.mediaPresent=false;this.imagesetter.supplyBrakeActive=false;this.imagesetter.tensionValid=false;this.imagesetter.capstanEncoderActive=false;this.imagesetter.polygonAtSpeed=false;this.imagesetter.exposurePermit=false;this.imagesetter.exposureActive=false;this.imagesetter.exposureComplete=false;this.imagesetter.cutterPermit=false;this.imagesetter.cutterHomeConfirmed=true;this.imagesetter.cuttingActive=false;this.imagesetter.outputDetected=false;this.imagesetter.interlockSafe=true;this.imagesetter.capstanAdvanceActive=false;this.imagesetter.tensionRegulationActive=false;this.imagesetter.outputBoundaryActive=false;this.imagesetter.mediaContactStage='SUPPLY';if(this.imagesetter.laser?.material?.emissive){this.imagesetter.laser.material.emissive.setHex(0);this.imagesetter.laser.material.emissiveIntensity=0;}}if(this.zund){if(this.zund.beam)this.zund.beam.position.copy(this.zund.beamRest);if(this.zund.carriage)this.zund.carriage.position.copy(this.zund.carriageRest);this.zund.vacuumHoldActive=false;this.zund.vacuumControlActive=false;this.zund.vacuumMode='RELEASE';this.zund.vacuumContactActive=false;this.zund.toolActionActive=false;this.zund.toolClearanceMaintained=true;}if(this.zundMaterial){this.zundMaterial.position.set(0,.69,0);this.zundMaterial.visible=this.active;}for(const p of this.compressorParticles)p.mesh.visible=this.active&&(p.kind!=='distribution'||this.pathVisible);if(this.compressor){this.compressor.intakeActive=this.active;this.compressor.compressionActive=this.active;this.compressor.separationActive=this.active;this.compressor.aftercoolingActive=this.active;this.compressor.distributionActive=this.active;this.compressor.oilCircuitActive=this.active;this.compressor.condensateDrainActive=this.active;this.compressor.pressureNormalizedPct=this.active?95:0;this.compressor.pressureProfile=this.active?{package:99,receiver:98.4,afterTreatment:95.8,ringNear:95,ringFar:94,service:[93.3,92.8,92.2]}:{package:0,receiver:0,afterTreatment:0,ringNear:0,ringFar:0,service:[0,0,0]};}for(const p of this.ahuParticles)p.mesh.visible=this.active&&(p.kind!=='return'||this.pathVisible);if(this.ahu){this.ahu.supplyAirActive=this.active;this.ahu.returnAirReferenceActive=this.active;this.ahu.outdoorHeatRejectionActive=false;this.ahu.evaporativePrecoolActive=false;this.ahu.dxEvaporatorActive=false;this.ahu.genericCoilActive=false;}if(this.folderBlank){this.folderBlank.position.set(-5.20,.82,0);this.folderBlank.rotation.set(0,0,0);this.folderBlank.visible=this.active;}if(this.folder){this.folder.leftPivot.rotation.x=0;this.folder.rightPivot.rotation.x=0;this.folder.glueStrip.visible=false;this.folderBlank.scale.set(1,1,1);this.folder.foldingActive=false;this.folder.prebreakActive=false;this.folder.alignmentActive=false;this.folder.glueZoneActive=false;this.folder.finalFoldActive=false;this.folder.compressionActive=false;this.folder.deliveryActive=false;this.folder.feederDriveActive=false;this.folder.foldDriveActive=false;this.folder.compressionDriveActive=false;this.folder.deliveryDriveActive=false;this.folder.beltContactActive=false;this.folder.materialContactZone=0;for(const m of Object.values(this.folder.drives||{}))if(m)m.quaternion.copy(m.userData.folderDriveRestQuaternion||new THREE.Quaternion());}}
 update(now){
