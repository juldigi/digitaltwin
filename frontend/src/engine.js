...
export class FactoryEngine {
  constructor(container,onSelect){
    ...
    {const requested=new URLSearchParams(location.search).get('machine');this.machineKey=['offset10','apm2'].includes(requested)||universalMachineConfig(requested)?requested:'offset5';}
    ...
    this.template=this.machineKey==='offset10'?new Offset10MachineTemplate():this.machineKey==='apm2'?new APM2MachineTemplate():generic?new UniversalMachineTemplate(this.machineKey):new OffsetMachineTemplate();this.machine=this.template.root;this.scene.add(this.machine);
    this.simulation=this.machineKey==='offset10'?new Offset10PrintingSimulation(this.machine,this.template):this.machineKey==='apm2'?new APM2ProcessSimulation(this.machine,this.template):generic?new UniversalProcessSimulation(this.machine,this.template):new PrintingSimulation(this.machine,this.template);this.simulation.onUpdate=state=>this.onSimulationUpdate?.(state);
