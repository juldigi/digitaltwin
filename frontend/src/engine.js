...
export class FactoryEngine {
  ...
  applyPlacement(state,layout=this.layout||state.layout){const a=state.asset,l=layout;this.machine.visible=false;this.primaryFactoryPlaced=false;if(!l)return;
    if(l.baselineId&&this.machineKey===FOUNDATION_SCOPE.primaryRoute){const p=this.primaryFactoryPlacement(l);if(p&&Number.isFinite(p.x)&&Number.isFinite(p.y)){const scale=Number.isFinite(p.scale)&&p.scale>0?p.scale:1,rotation=(Number(p.rotation)||0)*Math.PI/180;this.machine.updateWorldMatrix(true,true);const localBox=new THREE.Box3().setFromObject(this.machine),localCenter=localBox.getCenter(new THREE.Vector3()),horizontalAnchor=new THREE.Vector3(localCenter.x,0,localCenter.z).multiplyScalar(scale).applyAxisAngle(new THREE.Vector3(0,1,0),rotation);this.machine.scale.setScalar(scale);this.machine.rotation.y=rotation;this.machine.position.set(p.x-horizontalAnchor.x,(Number.isFinite(p.z)?p.z:0)-localBox.min.y*scale,-p.y-horizontalAnchor.z);this.primaryFactoryPlaced=true;this.machine.userData={...this.machine.userData,factoryIntegrated:true,factoryMachineId:FOUNDATION_SCOPE.primaryMachineId,factoryPlacementStatus:p.status||'UNKNOWN',factoryScaleFitApplied:false,factoryAnchorMode:'DXF_FOOTPRINT_CENTER_TO_MODEL_BOUNDS',factoryLocalBoundsCenter:[localCenter.x,localCenter.y,localCenter.z],factoryLocalFloorY:localBox.min.y};this.syncPrimaryFactoryRepresentation();}return;}
