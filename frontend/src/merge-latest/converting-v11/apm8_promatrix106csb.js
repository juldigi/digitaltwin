 ...
 * APM-8 — AUTOPLATEN 8 STRIPPING & BLANKING — Promatrix 106 CSB
 * Machine-local geometry/state/animation; do not replace with a shared generic machine.
 * SOURCE: https://www-server1.heidelberg.com/global/en/print_and_packaging/finishing/die_cutting/die_cutting__machines/promatrix_106_csb/promatrix_106_csb_1.jsp
 */
(function(global){
'use strict';
const MACHINE_ID="APM-8";
const SPEC={
  "maxSheet": [
    1060,
    760
...
const ENVELOPE_M=[8.15, 4.45, 2.7];
const STATIONS=[
  {
    "id": "feeder",
  ...
  },
  {
    ...
    "height": 1.55,
    "components": [
      "top/middle/bottom quick-lock tools",
      "fine adjustment",
      "vacuum device",
  ...
  {
    "id": "delivery",
    "label": "Automatic pallet delivery",
    "x": 6.85,
    "height": 1.78,
    "components": [
      "rake system",
...
const VISUAL_DNA={
  "operatorSide": "tool-change access aligned to cutting/stripping/blanking stations",
  "driveSide": "chain drive, vacuum/waste services and cabinets",
  "support": "rigid long bed with supported waste penetrations"
};
...
function addStations(T,root,M){STATIONS.forEach((st,i)=>{const g=new T.Group();g.name=st.id;g.position.x=st.x;root.add(g);const w=i===STATIONS.length-1?1.05:Math.max(.85,(STATIONS[i+1]?.x??st.x+1.2)-st.x-.1);const zs=Math.min(ENVELOPE_M[1]*.72,1.75);box(T,st.id+'_upright_os',[.11,st.height,.11],[.12,st.height/2,zs/2],M.frame,g);box(T,st.id+'_upright_ds',[.11,st.height,.11],[.12,st.height/2,-zs/2],M.frame,g);box(T,st.id+'_top_os',[w,.11,.11],[w/2,st.height,zs/2],M.frame,g);box(T,st.id+'_top_ds',[w,.11,.11],[w/2,st.height,-zs/2],M.frame,g);const y=0.88;box(T,st.id+'_deck',[w,.09,zs*.94],[w/2,y,0],M.dark,g);for(let k=0;k<4;k++)cyl(T,st.id+'_roller_'+k,.045,Math.max(.55,zs*.84),[.18+k*(w-.3)/3,y+.08,0],[Math.PI/2,0,0],M.roller,g);if(['diecutter','blanker','foil'].includes("blanker")){box(T,st.id+'_os_guard',[w*.82,Math.max(.42,st.height-y-.18),.065],[w*.52,y+(st.height-y)/2,zs/2+.1],M.panel,g);box(T,st.id+'_window',[w*.42,Math.max(.18,(st.height-y)*.28),.018],[w*.5,y+.4,zs/2+.14],M.glass,g);}if("blanker"==='foldergluer'){for(let b=0;b<5;b++)box(T,st.id+'_belt_'+b,[w*.94,.018,.07],[w*.5,y+.045,(b-2)*.19],M.belt,g);box(T,st.id+'_adjust_bridge',[.075,.65,1.42],[w*.52,y+.36,0],M.frame,g);}if("blanker"==='inspection'&&st.id.includes('inspect')){box(T,st.id+'_hood',[w*.95,.82,1.52],[w*.5,y+.54,0],M.dark,g);box(T,st.id+'_camera_gantry',[w*.76,.055,1.3],[w*.5,y+.79,0],M.frame,g);for(let c=0;c<3;c++)box(T,st.id+'_camera_'+c,[.14,.12,.18],[w*(.3+.2*c),y+.7,0],M.dark,g);}if("blanker"==='foil'&&st.id.includes('foil')){for(let r=0;r<4;r++){const rx=w*(.22+.18*r);box(T,st.id+'_reel_support_'+r,[.07,.92,.07],[rx,y+.48,.62],M.frame,g);cyl(T,st.id+'_reel_'+r,.13,.5,[rx,y+.88,0],[Math.PI/2,0,0],M.roller,g);}}});}
...
function build(T,options={}){if(!T)throw new Error(MACHINE_ID+' requires THREE');const root=new T.Group();root.name=MACHINE_ID;root.userData={machineId:MACHINE_ID,name:"AUTOPLATEN 8 STRIPPING & BLANKING",model:"Promatrix 106 CSB",spec:SPEC,taxonomy:taxonomy(),motions:MOTIONS,visualDNA:VISUAL_DNA,evidenceStatus:"REFERENCE_VERIFIED_MODEL_FAMILY"};const mm=materials(T);addBase(T,root,mm);addStations(T,root,mm);addServices(T,root,mm);return root;}
function smooth(a,b,x){const t=Math.max(0,Math.min(1,(x-a)/(b-a)));return t*t*(3-2*t);}
function apply(root,p,s){root.userData.sim={phase:p,sheetProgress:p,gripperProgress:p,pressLift:Math.max(0,Math.sin(Math.PI*2*p))**2,stripLift:Math.max(0,Math.sin(Math.PI*2*(p-.12)))**2,blankLift:Math.max(0,Math.sin(Math.PI*2*(p-.24)))**2,foilIndex:p<.58?p/.58:1,beltPhase:(p*8)%1,rejectGate:0,cycle:s.cycle};if("blanker"==='inspection'){const bad=s.defects.has(s.productSeq);root.userData.sim.lightMode=Math.floor((p*4)%4);root.userData.sim.cameraAcquire=p>.34&&p<.58;root.userData.sim.rejectGate=bad&&p>.68&&p<.79?1:0;}if("blanker"==='foldergluer'){root.userData.sim.prefold=smooth(.18,.42,p);root.userData.sim.bottomFold=smooth(.34,.58,p);root.userData.sim.glueOn=p>.48&&p<.62;root.userData.sim.finalFold=smooth(.56,.82,p);root.userData.sim.compression=p>.76?smooth(.76,1,p):0;}}
