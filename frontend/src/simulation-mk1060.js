...
export class MK1060ProcessSimulation{
 constructor(root,template){
  ...
  this.blankUpper=template.findNode('mk1060-blank-upper');this.blankLower=template.findNode('mk1060-blank-lower');this.feederHead=template.findNode('mk1060-feeder-head');this.productDelivery=template.findNode('mk1060-product-delivery');this.wasteConveyor=template.findNode('mk1060-waste-conveyor');
  ...
  this.outputAnchors={product:new THREE.Vector3(2.58,.78,0),waste:new THREE.Vector3(2.33,.72,.56)};this.refreshOutputAnchors();this.pathVisible=false;this.inkFlowVisible=false;this.resetStateFlags();
 }
 refreshOutputAnchors(){this.root.updateMatrixWorld(true);const anchor=(node,fallback)=>{if(!node)return fallback;node.updateWorldMatrix(true,true);const box=new THREE.Box3().setFromObject(node);if(box.isEmpty())return fallback;const p=new THREE.Vector3((box.min.x+box.max.x)/2,box.max.y,(box.min.z+box.max.z)/2);this.root.worldToLocal(p);return p;};this.outputAnchors.product=anchor(this.productDelivery,this.outputAnchors.product);this.outputAnchors.waste=anchor(this.wasteConveyor,this.outputAnchors.waste);}
 ...
 start(){this.refreshOutputAnchors();this.active=true;this.running=true;this.paused=false;this.elapsed=0;this.lastNow=null;this.completed=0;for(const s of this.sheets)s.lap=-1;this.resetMechanisms();this.onUpdate?.(this.state());return this.state();}
 ...
 updateSheets(cycleIndex,transport){
  ...
  for(const s of this.sheets){const raw=global+s.phase,t=((raw%1)+1)%1;s.mesh.visible=true;s.mesh.position.copy(this.curve.getPointAt(Math.min(.999,t)));s.mesh.rotation.set(-Math.PI/2,0,0);
   ...
   const lap=Math.floor(raw);if(lap>s.lap){if(s.lap>=0){this.completed++;const idx=(this.completed-1)%this.blankStack.length,b=this.blankStack[idx],w=this.wastePieces[idx];b.visible=true;b.position.set(this.outputAnchors.product.x,this.outputAnchors.product.y+idx*.007,this.outputAnchors.product.z);w.visible=true;w.position.set(this.outputAnchors.waste.x,this.outputAnchors.waste.y+idx*.003,this.outputAnchors.waste.z);}s.lap=lap;}
  }
 }
 update(now){
