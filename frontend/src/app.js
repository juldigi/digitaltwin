...
import {validateSceneOverrides} from './scene-editor-state.js';
...
const normalizeMachineKey=key=>{const raw=String(key??'').trim();return raw?(LEGACY_MACHINE_ROUTE[raw]||raw):null;};
...
function configureActiveMachine(requested){
 ...
 if(!requested)throw new Error('Aset mesin harus dipilih sebelum membuka model 3D.');
...
function enableExteriorOpen({forceDetail=true}={}){
 ...
 engine.template.reset();engine.clearPartLabels();engine.isolated=false;if(forceDetail)engine.setLow(false);engine.template.setExteriorOpen(true);
...
function startPrintingSimulation(){
 ...
 if(!engine.isPrintingSimulationActive()){
  const readiness=engine.getPrintingSimulationState?.()||simulationState;
...
function renderStatus(){
 const l=activeLayout(),machineCount=$('#machine-count'),truth=layoutTruth(l),placement=placementForMachine?.(machineRecordForRoute(MACHINE_KEY)?.machineId||FOUNDATION_SCOPE.primaryMachineId)||null,assetStatus=assetTruth(state?.asset,{placement,sourceCount:TECHNICAL_SOURCES.length});
...
function setView(view){
 ...
 const viewLayoutTruth=layoutTruth(l),viewAssetTruth=assetTruth(state?.asset,{placement:placementForMachine?.(machineRecordForRoute(MACHINE_KEY)?.machineId||FOUNDATION_SCOPE.primaryMachineId)||null,sourceCount:TECHNICAL_SOURCES.length});
...
function connectionDialog(){
 ...
 on('#superadmin-login',()=>{const loginBase=($('#api-base')?.value||apiBase||location.origin).trim();modal('Masuk Superadmin',`<form id="superadmin-form"><p>Masuk dengan kata sandi Superadmin. Gunakan kata sandi Superadmin Anda. Setelah masuk pertama kali, segera ganti kata sandi di Pengaturan.</p><label for="superadmin-password">Kata sandi</label><input id="superadmin-password" type="password" autocomplete="current-password" required><div class="actions"><button type="submit" class="primary">Masuk Superadmin</button></div><p id="superadmin-error" class="inline-error" role="alert"></p></form>`);$('#superadmin-form').onsubmit=async event=>{event.preventDefault();const button=event.target.querySelector('button');button.disabled=true;try{const base=new URL(loginBase,location.href).origin;const login=await request('/api/superadmin/login',{method:'POST',base,key:'',data:{password:$('#superadmin-password').value}});const next=await request('/api/state',{base,key:login.token});apiBase=base;token=login.token;role='superadmin';localStorage.setItem('offset5-api-base',base);await acceptState(next);updateConnectionTruth();closeModal();toast(login.passwordChangeRequired?'Masuk berhasil. Segera ganti kata sandi di Pengaturan.':'Masuk Superadmin berhasil.');}catch(error){$('#superadmin-error').textContent=error.message;}finally{button.disabled=false;}};});
 on('#disconnect',async()=>{token='';role=null;await cache.clear();cacheEnabled=false;cachedDataActive=false;localStorage.removeItem('offset5-cache-enabled');state=structuredClone(initialState);engine?.loadLayout(bundledLayout);showHome({historyMode:'none'});renderStatus();updateConnectionTruth();closeModal();toast('Mode lokal aktif.');});
}
...
function foundationStatusDialog(){
 ...
 const placeholders=mapped.filter(p=>!canOpenTechnical3D(p.machineId));
...
function editorPanel(){
 ...
 if((role!=='admin'&&role!=='superadmin')){toast('Atur posisi memerlukan izin pengaturan.',true);return;}
...
function machineDetailDialog(machine){
 ...
 const copy=primary?'Buka model mesin untuk melihat struktur dan referensi sesuai bukti yang tersedia. Detail yang belum terverifikasi tetap ditandai.':'Model 3D belum tersedia untuk aset ini. Posisi pada denah tetap dapat diperiksa.';
...
function machineRoute(machine){
 return machine?.machineId==='BMJ-MCH-0009'?'offset10':machine?.machineId==='BMJ-MCH-0010'?'apm2':machine?.machineId==='BMJ-MCH-0002'?'sheeting':machine?.machineId==='BMJ-MCH-0003'?'offset5':machine?.machineId||null;
}
...
function openSceneEditor(){
 ...
 const overrides={...(state.sceneOverrides||{})};let selected=null,undo=[],redo=[],isolated=false,previewOriginal=false,history=[],editorScope='factory';
 const panel=document.createElement('section');panel.id='scene-editor-panel';panel.setAttribute('aria-label','Editor Scene 3D');panel.style.cssText='position:fixed;z-index:10000;right:12px;top:12px;bottom:12px;width:min(390px,calc(100vw - 24px));overflow:auto;background:#f7fafc;color:#182b36;border:1px solid #789;padding:16px;border-radius:14px;box-shadow:0 12px 45px #0008;font:14px system-ui';document.body.append(panel);
 const snapshot=()=>{undo.push(JSON.stringify(overrides));if(undo.length>50)undo.shift();redo=[];};
 const apply=()=>{if((selected?.startsWith('machine:')||selected?.startsWith('part:'))||Object.keys(overrides).some(id=>id.startsWith('copy:')||id.startsWith('new:'))||[...baseline.keys()].some(id=>id.startsWith('copy:')||id.startsWith('new:'))){engine.applySceneOverrides(overrides);if(selected&&engine.sceneObjects.has(selected)&&!previewOriginal)engine.selectSceneObject(selected);}else for(const [id,v] of baseline){const node=engine.sceneObjects.get(id);if(!node)continue;const draft=overrides[id],x=draft?.identity&&draft.identity!==engine.sceneIdentity(id)?v:draft||v;node.position.fromArray(x.position);node.rotation.set(...x.rotation);node.scale.fromArray(x.scale);node.visible=x.visible&&!x.deleted;}engine.sceneOverrides=overrides;if(isolated&&selected){let node=engine.sceneObjects.get(selected);while(node?.parent&&node!==engine.factory){for(const sibling of node.parent.children)if(sibling!==node)sibling.visible=false;node=node.parent;}}};
 ...
 const update=()=>{const queryBefore=panel.querySelector('#se-search')?.value||'';const v=current(),info=selected?engine.sceneObjectInfo(selected):null,wallId=selected?engine.sceneWallId(selected):null,wallPoints=wallId===selected?engine.sceneWallEndpoints(selected):null;panel.innerHTML=`<header style="display:flex;justify-content:space-between;align-items:center"><strong>Editor Scene 3D</strong><button id="se-close">Tutup</button></header><p>Pilih objek di 3D atau cari di daftar. Angka transform menggunakan koordinat lokal objek.</p><div class="actions"><button data-se-scope="factory" ${editorScope==='factory'?'class="active"':''}>Pabrik</button><button data-se-scope="machine" ${editorScope==='machine'?'class="active"':''}>Komponen ${esc(engine.machineKey)}</button></div>${editorScope==='factory'?'<div class="actions"><button data-se-add="box">Tambah Kotak</button><button data-se-add="cylinder">Tambah Silinder</button></div>':''}${engine.staleSceneOverrides?.length?`<p role="alert">${engine.staleSceneOverrides.length} override lama tidak cocok dengan scene terbaru dan tidak diterapkan.</p>`:''}<input id="se-search" type="search" placeholder="Cari objek, dinding, mesin…" style="width:100%"><select id="se-object" size="7" style="width:100%;margin:8px 0"></select><p id="se-selected">${esc(selected||'Pilih objek')} · ${selected?esc(engine.sceneObjects.get(selected)?.name||engine.sceneObjects.get(selected)?.userData?.semantic||'Bagian 3D'):''}</p>${selected?'<button id="se-parent">Pilih induk objek</button>':''}${wallId&&wallId!==selected?'<button id="se-wall">Edit segmen dinding beserta panelnya</button>':''}<label><input id="se-snap" type="checkbox" ${engine.gizmo.translationSnap?'checked':''}> Snap posisi 10 cm / rotasi 5°</label><div class="actions"><button id="se-focus">Fokus</button><button id="se-isolate">${isolated?'Tampilkan Semua':'Isolasi sementara'}</button></div><div class="edit-strip">${['translate','rotate','scale'].map((m,i)=>`<button data-se-mode="${m}" ${engine.gizmo.mode===m?'class="active"':''}>${['Geser','Putar','Skala'][i]}</button>`).join('')}</div>${info?`<p>Ukuran (m): ${info.dimensions.map(n=>n.toFixed(2)).join(' × ')}</p>${info.collisions.length?`<p role="alert">Potensi overlap aset: ${esc(info.collisions.join(', '))}</p>`:''}`:''}${wallPoints?`<fieldset><legend>Titik ujung dinding (X / Z meter)</legend>${wallPoints.map((point,index)=>`<label>${index?'B':'A'} X <input type="number" step="0.01" data-wall-point="${index}" data-wall-axis="0" value="${point[0].toFixed(3)}"></label><label>${index?'B':'A'} Z <input type="number" step="0.01" data-wall-point="${index}" data-wall-axis="1" value="${point[1].toFixed(3)}"></label>`).join('')}<p>Panel, girt, kaca, dan flashing bergerak bersama segmen. Periksa pintu serta bukaan setelah mengubah dinding.</p></fieldset>`:''}<div id="se-numbers">${v?['position','rotation','scale'].map(key=>`<fieldset><legend>${key}</legend>${v[key].map((n,i)=>`<label>${'XYZ'[i]} <input data-se-key="${key}" data-se-axis="${i}" type="number" step="0.01" value="${n.toFixed(3)}" ${v.locked?'disabled':''} style="width:75px"></label>`).join('')}</fieldset>`).join('')+`<button id="se-hide">${v.visible?'Sembunyikan':'Tampilkan'}</button><button id="se-delete">${v.deleted?'Pulihkan objek':'Hapus sementara'}</button><button id="se-lock">${v.locked?'Buka kunci':'Kunci objek'}</button><button id="se-ground">Turunkan ke lantai</button><button id="se-reset">Reset objek</button><button id="se-duplicate">Duplikat</button><fieldset><legend>Sejajarkan pusat dengan aset</legend><select id="se-align-target"><option value="">Pilih aset acuan</option>${[...engine.actualFactory.assets].map(([id,node])=>`<option value="asset:${esc(id)}">${esc(node.name||id)}</option>`).join('')}</select><button data-se-align="x">Sumbu X</button><button data-se-align="z">Sumbu Z</button><button data-se-align="xz">X dan Z</button></fieldset><div class="se-nudge" aria-label="Gerakkan objek dengan tombol"><button data-se-nudge="x:-1">← X</button><button data-se-nudge="x:1">X →</button><button data-se-nudge="z:-1">↓ Z</button><button data-se-nudge="z:1">Z ↑</button><button data-se-nudge="y:-1">− Tinggi</button><button data-se-nudge="y:1">+ Tinggi</button><button data-se-turn="-1">↶ 5°</button><button data-se-turn="1">↷ 5°</button></div>`:''}</div><div class="actions"><button id="se-undo">Undo</button><button id="se-redo">Redo</button><button id="se-save" class="primary">Simpan Scene</button><button id="se-preview">${previewOriginal?'Kembali ke Draft':'Lihat Asli'}</button><button id="se-history">Riwayat</button><button id="se-export">Ekspor JSON</button><label>Impor JSON<input id="se-load-file" type="file" accept="application/json,.json"></label></div><p id="se-status">${previewOriginal?'Tampilan asli · mode baca':Object.keys(overrides).length+' override dalam draft'}</p><div id="se-history-list" hidden></div>`;
 const choices=[...engine.sceneObjects].filter(([id,node])=>!(id.startsWith('node:')&&node.userData.editorStableId)&&!(id.startsWith('machine:')&&node.userData.nodeId&&engine.sceneObjects.has('part:'+engine.machineKey+':'+node.userData.nodeId))).filter(([id])=>editorScope==='machine'?(id.startsWith('machine:'+engine.machineKey+':')||id.startsWith('part:'+engine.machineKey+':')):!id.startsWith('machine:')&&!id.startsWith('part:')).map(([id,node])=>{const lineage=[];for(let parent=node.parent;parent&&lineage.length<2;parent=parent.parent)if(parent.name)lineage.unshift(parent.name);return [id,[...lineage,node.name||node.userData?.semantic||'Objek 3D'].join(' / ')];});const fill=()=>{const query=panel.querySelector('#se-search').value.toLowerCase();panel.querySelector('#se-object').innerHTML=choices.filter(([id,name])=>(id+' '+name).toLowerCase().includes(query)).slice(0,300).map(([id,name])=>`<option value="${esc(id)}" ${id===selected?'selected':''}>${esc(name)} · ${esc(id)}</option>`).join('');};panel.querySelector('#se-search').value=queryBefore;fill();panel.querySelector('#se-search').oninput=fill;panel.querySelector('#se-object').onchange=e=>engine.selectSceneObject(e.target.value);panel.querySelector('#se-snap').onchange=e=>{engine.gizmo.setTranslationSnap(e.target.checked?.1:null);engine.gizmo.setRotationSnap(e.target.checked?Math.PI/36:null);};panel.querySelector('#se-focus').onclick=()=>{if(selected)engine.fit(engine.sceneObjects.get(selected));};panel.querySelector('#se-isolate').onclick=()=>{if(!selected)return;isolated=!isolated;apply();if(isolated){let node=engine.sceneObjects.get(selected);while(node?.parent&&node!==engine.factory){for(const sibling of node.parent.children)if(sibling!==node)sibling.visible=false;node=node.parent;}}update();};
 panel.querySelectorAll('[data-se-mode]').forEach(b=>b.onclick=()=>{engine.gizmo.setMode(b.dataset.seMode);update();});panel.querySelectorAll('[data-se-key]').forEach(input=>input.onchange=()=>{if(previewOriginal||overrides[selected]?.locked)return;const value=+input.value;if(!Number.isFinite(value)||Math.abs(value)>100000||input.dataset.seKey==='scale'&&value<=0)return toast('Angka tidak valid.',true);snapshot();const v=current();v[input.dataset.seKey][+input.dataset.seAxis]=value;overrides[selected]=v;apply();update();});
 ...
 panel.querySelector('#se-preview').onclick=()=>{previewOriginal=!previewOriginal;if(previewOriginal){engine.gizmo.detach();engine.applySceneOverrides({});}else{apply();if(selected&&!overrides[selected]?.locked)engine.gizmo.attach(engine.sceneObjects.get(selected));}update();};
 ...
 panel.querySelector('#se-undo').onclick=()=>{if(!undo.length)return;redo.push(JSON.stringify(overrides));Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,JSON.parse(undo.pop()));apply();update();};
 panel.querySelector('#se-redo').onclick=()=>{if(!redo.length)return;undo.push(JSON.stringify(overrides));Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,JSON.parse(redo.pop()));apply();update();};
 panel.querySelector('#se-close').onclick=()=>{Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,state.sceneOverrides||{});apply();engine.applySceneOverrides(state.sceneOverrides||{});engine.gizmo.setTranslationSnap(null);engine.gizmo.setRotationSnap(null);engine.setSceneEditing(false);engine.onSceneSelect=null;engine.onSceneTransform=null;engine.gizmo.removeEventListener('mouseDown',onDragStart);engine.gizmo.removeEventListener('mouseUp',onDragEnd);panel.remove();};
 panel.querySelector('#se-save').disabled=previewOriginal;panel.querySelector('#se-save').onclick=async()=>{if(previewOriginal)return;try{const next=await request('/api/scene',{method:'PUT',data:{overrides}});await acceptState(next);toast('Scene disimpan.');}catch(e){panel.querySelector('#se-status').textContent=e.message;}};
 panel.querySelector('#se-export').onclick=()=>{const blob=new Blob([JSON.stringify({schemaVersion:1,overrides},null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='bmj-scene-overrides.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),30000);};
 panel.querySelector('#se-load-file').onchange=async e=>{try{const file=e.target.files?.[0];if(!file)return;if(file.size>1024*1024)throw new Error('Berkas melebihi 1 MB.');const parsed=JSON.parse(await file.text());if(parsed.schemaVersion!==1||!parsed.overrides||typeof parsed.overrides!=='object'||Array.isArray(parsed.overrides))throw new Error('Format override tidak valid.');const entries=Object.entries(validateSceneOverrides(parsed.overrides));if(entries.some(([id,v])=>!engine.sceneObjects.has(id)||v.identity&&v.identity!==engine.sceneIdentity(id)))throw new Error('Berkas berisi objek yang tidak sesuai versi scene saat ini.');snapshot();Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,parsed.overrides);apply();update();}catch(error){toast(error.message,true);}};
 panel.querySelector('#se-history').onclick=async()=>{try{const response=await request('/api/scene/revisions');history=response.revisions||[];const list=panel.querySelector('#se-history-list');list.hidden=false;list.innerHTML='<h4>Revisi sebelumnya</h4>'+history.slice().reverse().map((item,index)=>`<button data-se-revision="${esc(item.id)}">${esc(new Date(item.at).toLocaleString('id-ID'))} · ${item.objects??Object.keys(item.overrides||{}).length} objek · ${esc(item.by||'Superadmin')}</button>`).join('')+'<p>Memulihkan revisi langsung menyimpan perubahan dan membuat revisi cadangan baru.</p>';list.querySelectorAll('[data-se-revision]').forEach(button=>button.onclick=async()=>{try{const next=await request('/api/scene/restore',{method:'PUT',data:{id:button.dataset.seRevision}});await acceptState(next);Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,next.sceneOverrides||{});apply();update();toast('Revisi berhasil dipulihkan.');}catch(error){toast(error.message,true);}});}catch(e){toast(e.message,true);}};
...
on('#tool-simulation',()=>{const context=window.BMJAppState?.getState?.();if(!engine&&canOpenTechnical3D(context?.selectedAsset)){showPanel();renderPanel('simulation');return;}if(engine?.view!=='machine'||!canOpenTechnical3D(context?.selectedAsset)){dispatchEvent(new CustomEvent('bmj:machinecontextrequest',{detail:{selectedAsset:context?.selectedAsset,action:'Simulasi'}}));return;}showPanel();renderPanel('simulation');});
...
on('#tool-interior',()=>{if(!ensureMachineInspectionContext('Buka Interior'))return;if(exteriorMode)resetExteriorView();else showExteriorAll();$('#tool-interior')?.classList.toggle('active',exteriorMode);emitDomainState({inspectionMode:{interior:exteriorMode}});});
