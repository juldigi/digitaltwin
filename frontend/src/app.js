...
function openSceneEditor(){
 ...
 const overrides={...(state.sceneOverrides||{})};let selected=null,undo=[],redo=[];
 ...
 const apply=()=>{for(const [id,v] of baseline){const node=engine.sceneObjects.get(id);if(!node)continue;const x=overrides[id]||v;node.position.fromArray(x.position);node.rotation.set(...x.rotation);node.scale.fromArray(x.scale);node.visible=x.visible;}engine.sceneOverrides=overrides;};
 ...
 const update=()=>{const v=current();panel.innerHTML=`<header style="display:flex;justify-content:space-between;align-items:center"><strong>Editor Scene 3D</strong><button id="se-close">Tutup</button></header><p>Pilih objek di 3D atau cari di daftar. Perubahan berlaku setelah Simpan.</p><input id="se-search" type="search" placeholder="Cari objek, dinding, mesin…" style="width:100%"><select id="se-object" size="7" style="width:100%;margin:8px 0"></select><p id="se-selected">${esc(selected||'Pilih objek')}</p><div class="edit-strip">${['translate','rotate','scale'].map((m,i)=>`<button data-se-mode="${m}" ${engine.gizmo.mode===m?'class="active"':''}>${['Geser','Putar','Skala'][i]}</button>`).join('')}</div><div id="se-numbers">${v?['position','rotation','scale'].map(key=>`<fieldset><legend>${key}</legend>${v[key].map((n,i)=>`<label>${'XYZ'[i]} <input data-se-key="${key}" data-se-axis="${i}" type="number" step="0.01" value="${n.toFixed(3)}" style="width:75px"></label>`).join('')}</fieldset>`).join('')+`<button id="se-hide">${v.visible?'Sembunyikan':'Tampilkan'}</button><button id="se-ground">Turunkan ke lantai</button><button id="se-reset">Reset objek</button>`:''}</div><div class="actions"><button id="se-undo">Undo</button><button id="se-redo">Redo</button><button id="se-save" class="primary">Simpan Scene</button><button id="se-history">Riwayat</button></div><p id="se-status">${Object.keys(overrides).length} override tersimpan dalam draft.</p>`;
 const choices=[...engine.sceneObjects].filter(([id,node])=>id.startsWith('asset:')||node.isMesh&&node.name).map(([id,node])=>[id,node.name||id]);const fill=()=>{const query=panel.querySelector('#se-search').value.toLowerCase();panel.querySelector('#se-object').innerHTML=choices.filter(([id,name])=>(id+' '+name).toLowerCase().includes(query)).slice(0,300).map(([id,name])=>`<option value="${esc(id)}" ${id===selected?'selected':''}>${esc(name)} · ${esc(id)}</option>`).join('');};fill();panel.querySelector('#se-search').oninput=fill;panel.querySelector('#se-object').onchange=e=>engine.selectSceneObject(e.target.value);
 ...
 panel.querySelector('#se-close').onclick=()=>{Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,state.sceneOverrides||{});apply();engine.setSceneEditing(false);engine.onSceneSelect=null;engine.onSceneTransform=null;engine.gizmo.removeEventListener('mouseDown',onDragStart);engine.gizmo.removeEventListener('mouseUp',onDragEnd);panel.remove();};
