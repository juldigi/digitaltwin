...
function openSceneEditor(){
 ...
 const editorLabel=(id,node)=>{
  ...
  if(category==='machines'){const machineId=id.replace(/^asset:/,'')||node?.userData?.machineId,record=MACHINE_REGISTRY_BY_ID.get(machineId);return record?.name||record?.sapCode||node?.userData?.machineId||'Mesin';}
 ...
 <section class="se-step">
  ...
  ${selected?`<div class="se-selected-card"><div><small>TERPILIH</small><strong>${esc(selectedName)}</strong><span>${selectedMachine?esc([selectedMachine.sapCode,selectedMachine.model].filter(Boolean).join(' · ')||'Mesin dari database'):esc(editorCategories.find(category=>category.id===editorCategory)?.label||'Objek pabrik')}</span></div><button id="se-focus">Lihat dekat</button></div>${selectedMachine?'<div class="se-machine-edit-note"><strong>Seluruh mesin aktif sebagai satu unit</strong><span>Geser atau putar akan memindahkan seluruh mesin, bukan bagian kecilnya. Gunakan tombol panah keyboard untuk menggeser.</span></div>':''}`:'<div class="se-empty-selection"><strong>Pilih satu objek</strong><span>Pilih kategori di atas, lalu pilih objek dari daftar atau klik langsung pada tampilan 3D.</span></div>'}
 </section>
 ...
 const onEditorKeyDown=e=>{
  if(!selected?.startsWith('asset:')||previewOriginal||overrides[selected]?.locked||!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key))return;
  ...
  e.preventDefault();snapshot();const step=e.shiftKey?.5:.1;
  if(e.key==='ArrowLeft')node.position.x-=step;
  if(e.key==='ArrowRight')node.position.x+=step;
  if(e.key==='ArrowUp')node.position.z-=step;
  if(e.key==='ArrowDown')node.position.z+=step;
  overrides[selected]=current();engine.sceneOverrides=overrides;update();
 };
 ...
 panel.querySelector('#se-close').onclick=()=>{if(JSON.stringify(overrides)!==JSON.stringify(state.sceneOverrides||{})&&!confirm('Perubahan yang belum disimpan akan dibuang. Tutup Editor?'))return;isolated=false;isolationGuard.restore();Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,state.sceneOverrides||{});engine.applySceneOverrides(state.sceneOverrides||{});engine.gizmo.setTranslationSnap(null);engine.gizmo.setRotationSnap(null);engine.setSceneEditing(false);engine.onSceneSelect=null;engine.onSceneTransform=null;engine.gizmo.removeEventListener('mouseDown',onDragStart);engine.gizmo.removeEventListener('mouseUp',onDragEnd);document.removeEventListener('keydown',onEditorKeyDown);panel.remove();};
