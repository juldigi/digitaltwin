...
function applyActiveMachineState(){
 ...
 if(IS_SHEETING)state.asset={...state.asset,asset_id:'BMJ-MCH-0002',asset_code:'SBM-2',codename:'SBM-2',model:'HSM-CTM7',description:'SHEETING LEXUS',source_description:'BMJ Machine Database',manufacturer:'LEXUS',specification:'HSM-CTM7 · BMJ actual photo reconstruction · 2014',configuration:'Fixed-position hydraulic rollstand → low entry guide roll → separate open roller frame → LEXUS guarded cutter → multi-level green belt delivery → rack-adjusted stack table','3d_status':'DEDICATED PROCEDURAL / BMJ PHOTO-GROUNDED RECONSTRUCTION',data_confidence:'IDENTITY VERIFIED / ACTUAL PHOTO GEOMETRY / CUTTER INTERNAL UNRESOLVED',discovery_status:'BMJ_ACTUAL_PHOTOSET_PRIMARY__HSM_FAMILY_PROCESS_SECONDARY',serial_number:'00982',functional_location:'PC-PK2-OFS-SHT-SHEETING01',year:2014,sources:SHEETING_TECHNICAL_SOURCES};
...
function applyMachineShell(){
 ...
 if(IS_OFFSET10){
  ...
  const qgrid=$('[data-workbench-card="quick"] .quick-grid');if(qgrid)qgrid.innerHTML='<dt>Model plant</dt><dd>HSM-CTM7</dd><dt>Serial</dt><dd>00982</dd><dt>Tahun</dt><dd>2014</dd><dt>SAP Code</dt><dd>SBM-2</dd><dt>Referensi visual</dt><dd>9 foto aktual BMJ IMG_2479–IMG_2487</dd><dt>HSM 56 pembanding</dt><dd>Process/spec only · 300 m/min · 600 gsm</dd><dt>Mekanisme cutter internal</dt><dd>Tertutup / tidak direka</dd><dt>Alur aktual</dt><dd>RIGHT → LEFT</dd>';
  const warning=$('#dwg-warning');if(warning)warning.textContent='Sheeting V195 memakai foto aktual BMJ sebagai visual source of truth: hydraulic rollstand kanan → low entry roll → roller frame terpisah → guarded LEXUS cutter → delivery → rack stacker kiri. Sambungan yang tidak terlihat tidak direka.';
 }else if(IS_GENERIC){
