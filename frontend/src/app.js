...
function updateEvidenceStatus(){
 ...
 detail.innerHTML=`${pair('Identitas aset',identity||'UNKNOWN')}${pair('Status operasi',truth.operatingStatus)}${pair('Nilai kesehatan mesin',truth.healthScore)}${pair('Dasar model 3D',truth.source3D)}${pair('Detail model 3D',truth.detail3D)}${pair('Keandalan data',truth.dataConfidence)}${pair('Posisi',truth.position)}${pair('Foto aktual / daftar mesin',photos?photos+' foto':'UNKNOWN')}${pair('Dokumen / sumber teknis',docs?docs+' sumber':'UNKNOWN')}`;
}
function applyMachineShell(){
 ...
 if(IS_OFFSET10){
  ...
  const warning=$('#dwg-warning');if(warning)warning.textContent='Model Offset 10 mengikuti denah mesin final Heidelberg. Posisi pabriknya tidak disamakan dengan Offset 5.';
 }else if(IS_APM2){
  const mark=$('.brandmark');if(mark)mark.innerHTML='A<span>2</span>';
  const icon=$('.asset-icon');if(icon)icon.textContent='A2';
  const kpi=$('.top-kpis>div:nth-child(2)');if(kpi)kpi.innerHTML='<small>REFERENSI TEKNIS</small><b>'+TECHNICAL_SOURCES.length+'</b><span>Database BMJ dan acuan jenis mesin</span>';
  const warning=$('#dwg-warning');if(warning)warning.textContent='Posisi APM 2 di pabrik belum dapat dipastikan sebelum denah aktual diverifikasi.';
 }else if(IS_SHEETING){
  const mark=$('.brandmark');if(mark)mark.innerHTML='S<span>2</span>';
  const icon=$('.asset-icon');if(icon)icon.textContent='S2';
  const kpi=$('.top-kpis>div:nth-child(2)');if(kpi)kpi.innerHTML='<small>REFERENSI TEKNIS</small><b>'+TECHNICAL_SOURCES.length+'</b><span>Database BMJ dan acuan jenis mesin</span>';
  const warning=$('#dwg-warning');if(warning)warning.textContent='Model sheeting mengacu pada foto aktual BMJ. Penyangga roll menampilkan satu gulungan dengan lengan di kedua sisinya. Simulasi menunjukkan kertas dari web yang bergerak terus, dipotong menjadi lembar, lalu masuk ke tumpukan hasil. Bentuk pisau bagian dalam belum dipastikan karena tidak terlihat pada foto mesin BMJ.';
 }else if(IS_GENERIC){
  ...
  const warning=$('#dwg-warning');if(warning)warning.textContent='Posisi dan arah mesin ini di pabrik belum dapat dipastikan sebelum denah aktual diverifikasi.';
 }
}
...
function renderFactoryPanel(){
 ...
 const layerItems=[['roof','Atap'],['building','Lantai & isi ruangan'],['walls','Dinding'],['furniture','Furnitur'],['machines','Posisi aset'],['labels','Nama area & aset'],['unidentified','Area belum teridentifikasi'],['reference','Garis denah sumber']];
 $('#panel-content').innerHTML=`<h3>Denah pabrik</h3><p class="subtle">${known.length} posisi terpetakan · ${unknown.length} menunggu identifikasi posisi.</p><div class="card">${layerItems.map(([key,text])=>`<label class="check"><input type="checkbox" data-factory-layer="${key}" ${layers[key].visible?'checked':''}> ${text}</label>`).join('')}</div><label for="factory-asset-focus">Cari posisi aset</label><select id="factory-asset-focus"><option value="">Pilih posisi</option>${layout.placements.map(p=>`<option value="${p.machineId}">${esc(p.label)}${p.status==='UNIDENTIFIED'?' · belum teridentifikasi':''}</option>`).join('')}</select><div class="actions"><button id="factory-overview" class="secondary">Lihat seluruh pabrik</button><button id="factory-unknown" class="secondary">Posisi belum teridentifikasi</button></div><div class="card accent"><h4>Cakupan model dan sumber</h4><p>Denah DWG dan posisi aset yang dapat dikenali tetap ditampilkan. Detail 3D bergantung pada sumber tiap mesin. Jalur utilitas yang belum diperiksa di lapangan ditampilkan sebagai acuan, bukan jalur instalasi yang sudah pasti.</p></div><div class="card"><h4>Ketelitian tampilan</h4><p>Ketelitian denah, satuan dan skala, serta bagian yang belum ditampilkan dalam 3D dapat diperiksa pada Denah Pabrik → Ketelitian denah DWG. Nilai yang belum didukung sumber diberi keterangan sesuai tingkat kepastiannya.</p></div>`;
 const canonicalFactoryLayer={roof:'roof',building:'building',walls:'walls',furniture:'furniture',machines:'machines',labels:'labels',unidentified:'unidentified',reference:'reference'};
...
function renderPanel(tab=activeInspectorTab()){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Nama aset',a.description)+pair('Kode aset',a.asset_code)+pair('Kode nama',a.codename)+pair('Model',a.model)+pair('Kategori',a.category||'PRODUCTION_MACHINE')+pair('Subkategori',a.subcategory||'OFFSET_PRINTING')+pair('Pabrikan',a.manufacturer)+pair('Spesifikasi',a.specification)+pair('Lokasi',a.location)+pair('Status operasi',primaryTruth.operatingStatus)+pair('Nilai kesehatan mesin',primaryTruth.healthScore)}</dl><h3>Status model 3D</h3><dl class="data-list">${pair('Dasar model 3D',primaryTruth.source3D)+pair('Detail model 3D',primaryTruth.detail3D)+pair('Keandalan data',primaryTruth.dataConfidence)+pair('Posisi',primaryTruth.position)+pair('Jumlah sumber',primaryTruth.sourceCount?primaryTruth.sourceCount+' sumber':'UNKNOWN')}</dl><div class="card accent"><h4>Rekonstruksi berbasis denah + foto aktual</h4><p>Envelope mesin dan pitch berulang mengikuti footprint OFU-1 pada denah terkalibrasi. Bentuk luar mengacu pada foto aktual, sedangkan struktur fungsional internal mengacu pada dokumen CD102 yang tersedia.</p><span class="tag">PROCEDURAL / RECONSTRUCTED</span><span class="tag">Sebagian detail masih perkiraan</span></div><dl class="data-list">${pair('Envelope struktur',structuralText)+pair('Envelope termasuk area servis',serviceText)+pair('Pitch modul berulang',pitchText)+pair('Susunan unit','8 printing unit + coating + extension/delivery')+pair('Posisi di pabrik',primaryTruth.position)+pair('Dokumen identitas',a.sources.length+' dokumen')}</dl><div class="card"><h4>Batas ketelitian</h4><p>Ukuran envelope dan pitch berasal dari denah OFU-1. Dimensi internal seperti bearer, nip, cam timing, dan setelan roller hanya ditampilkan bila benar-benar didukung dokumen; sisanya tetap referensi visual.</p></div>`;
  }
  ...
  if(!engine){
   ...
   updateSimulationPanel(s);
    ...
    const processIntro=isAhu?'Simulasi AHU memperlihatkan udara masuk melalui filter dan cooling coil, lalu didorong fan ke saluran supply. Aliran return juga ditampilkan. Jalur saluran masih berupa acuan sampai gambar instalasi aktual tersedia.':isCompressor?'Simulasi compressor memperlihatkan udara masuk, dimampatkan di screw airend, dipisahkan dari oli, didinginkan, lalu disalurkan ke pipa distribusi. Jalur oli dan kondensat ditampilkan terpisah. Angka tekanan belum mewakili pembacaan aktual.':isCtp?'Simulasi CtP mengikuti urutan pemeriksaan pelat: pelat masuk, posisi terkunci pada drum, putaran sesuai, penyinaran laser berlangsung, lalu pelat dikeluarkan. Loader, punch, pembersih serpihan, dan processor belum diklaim terpasang pada mesin BMJ.':isCtf?'Simulasi CtF memperlihatkan media bergerak dengan tegangan terjaga, capstan dan laser bekerja setelah kondisi proses terpenuhi, lalu media terpotong dan keluar. Punch dan inline processor belum dipastikan terpasang.':isZund?'Simulasi Zünd memperlihatkan vacuum menahan material serta beam dan carriage mengikuti jalur kerja. Gerak potong, crease, dan router belum dijalankan karena alat yang terpasang pada mesin BMJ belum diketahui.':isCollator?'Simulasi collator mengambil lembar secara bertahap dari setiap bin, memeriksa masuknya kertas, lalu menggabungkannya dan mengirim hasil ke delivery. Jumlah bin dalam model merupakan acuan jenis mesin dan belum dipastikan sesuai unit BMJ.':isBlanker?'Simulasi blanker memperlihatkan tumpukan bergerak di atas platform, bergeser pada sumbu X/Y ke bawah hydraulic head, berhenti saat tekanan bekerja, lalu hasil dan sisa material dipisahkan.':isFolder?'Simulasi folder gluer memperlihatkan lembar masuk satu per satu, disejajarkan, dilipat, diberi perekat, ditekan, lalu dikirim ke delivery. Opsi crash-lock dan jenis glue applicator belum dipastikan terpasang.':'Simulasi memperlihatkan gerak mesin dan aliran material yang didukung sumber. Waktu dan posisi gerak hanya ilustrasi, bukan setelan produksi atau servis.';
    $('#panel-content').innerHTML=`<h3>Simulasi Proses · ${esc(m.name)}</h3><p class="subtle">${processIntro}</p><div class="card accent simulation-overview"><div class="simulation-status-row"><span id="sim-status" class="simulation-state">${s.running?'BERJALAN':s.active?'DIJEDA':'SIAP'}</span><b id="sim-stage">${esc(s.stage||PRINTING_SIMULATION_STAGES[0]||'Proses')}</b></div><div class="simulation-progress"><span id="sim-progress-bar" style="width:${Math.round((s.progress||0)*100)}%"></span></div><dl class="data-list compact"><dt>${isAhu||isCompressor?'Siklus visual':'Hasil selesai'}</dt><dd id="sim-completed">${s.completed||0}</dd><dt>${isAhu||isCompressor?'Partikel aliran udara':'Material bergerak'}</dt><dd id="sim-visible">${isAhu||isCompressor?(s.airflowParticleCount||0):(s.sheetsVisible||0)}</dd><dt>${isAhu?'Udara masuk / kembali':isCompressor?'Dalam mesin / distribusi':'Hasil di tumpukan / penampung'}</dt><dd id="sim-pile">${isAhu?((s.supplyAirflowParticleCount||0)+' / '+(s.returnAirflowParticleCount||0)):isCompressor?((s.compressedAirParticleCount||0)+' / '+(s.distributionAirParticleCount||0)):(s.pileSheetsVisible||0)}</dd>${pair(isAhu?'Mekanisme fan / actuator':isCompressor?'Drive / airend / fan':'Mekanisme terpetakan',s.mechanismCount||0)+numericRows+flagRows}</dl><div class="actions simulation-actions"><button id="sim-start" class="primary">${s.active?(s.running?'Running':'Lanjutkan'):'Mulai Simulasi Proses'}</button><button id="sim-pause" class="secondary">${s.running?'Jeda':'Lanjutkan'}</button><button id="sim-stop" class="secondary">Hentikan dan ulangi</button></div></div><h4>Kecepatan simulasi</h4><div class="simulation-speed">${[.5,1,1.5,2].map(v=>`<button data-sim-speed="${v}" class="${Math.abs((s.speed||1)-v)<.01?'active':''}">${v}×</button>`).join('')}</div><label class="check"><input id="sim-path" type="checkbox" ${s.pathVisible!==false?'checked':''}> ${isCompressor?'Tampilkan piping distribusi reference':isAhu?'Tampilkan ducting reference':'Tampilkan jalur referensi jika tersedia'}</label><h4>${isAhu?'Alur tata udara referensi':isCompressor?'Alur compressed-air reference':isCtp?'Alur external-drum imaging':isCtf?'Alur media & exposure':isZund?'Alur vacuum + XY toolpath':isCollator?'Alur suction & gathering':isBlanker?'Alur blanking X/Y + fixed head':isFolder?'Alur folding & gluing':'Unit proses yang tervalidasi'}</h4><div class="simulation-flow">${genericStages.map(stage=>`<div data-sim-stage="${esc(stage)}" class="${stage===s.stage?'active':''}"><span></span><b>${esc(stage)}</b></div>`).join('')}</div>${isAhu?'<div class="card"><h4>Batas routing duct</h4><p>Model memperlihatkan koneksi AHU → supply trunk → branch → diffuser serta return-air reference loop. Karena AHU 3–8 belum memiliki anchor posisi plant yang tervalidasi, ducting ini adalah functional airflow reference dan belum menjadi jalur as-built pabrik.</p></div>':''}${isCompressor?'<div class="card"><h4>Batas piping distribusi</h4><p>Model memperlihatkan discharge flexible connection, isolation/check valve, receiver boundary, dryer/filter boundary, closed-loop ring-main, service drops dan drip legs. Karena ketujuh compressor belum memiliki anchor posisi plant tervalidasi dan drawing compressed-air piping aktual belum tersedia, geometri ini adalah functional distribution reference—bukan jalur as-built. Receiver, dryer dan filter juga tetap explicit option boundary sampai instalasi aktual dikonfirmasi.</p></div>':''}<div class="card"><h4>Dasar informasi</h4><p>${esc(e.reason)}</p><span class="tag">${esc(e.grade)}</span><span class="tag">${esc(e.geometry)}</span></div>`;
    on('#sim-start',startPrintingSimulation);on('#sim-pause',pausePrintingSimulation);on('#sim-stop',()=>stopPrintingSimulation({restoreExterior:true}));
    document.querySelectorAll('[data-sim-speed]').forEach(b=>b.onclick=()=>{updateSimulationPanel(engine?.setPrintingSimulationSpeed(+b.dataset.simSpeed)||currentSimulationState());});
    const pathToggle=$('#sim-path');if(pathToggle)pathToggle.onchange=e=>{updateSimulationPanel(engine?.setPrintingSimulationPathVisible(e.target.checked)||currentSimulationState());};
    updateSimulationPanel(s);
  ...
  updateSimulationPanel(s);
   ...
   $('#panel-content').innerHTML=`<h3>Data aset</h3><p class="subtle">Nilai yang belum didukung sumber ditandai sebagai belum diketahui, belum diverifikasi, perkiraan, atau perlu ditinjau.</p><dl class="data-list">${rows.map(([k,v])=>pair(k,v)).join('')}</dl><div class="card"><h4>Konteks aktif</h4><p>${esc(meta?.name||'Mesin')} · posisi ${esc(readableStatus(truth.position))}</p></div>`;
...
function renderStatus(){
 ...
 $('#layout-status').textContent=l?`DWG · ${readableStatus(truth.planGeometry)}`:'DWG · Belum diketahui';
 $('#scale-status').textContent=`Skala · ${readableStatus(truth.scale)}`;
 $('#lod-status').textContent=engine?.view==='factory'?`Elevasi · ${readableStatus(truth.elevation)}`:`3D · ${readableStatus(assetStatus.source3D)}`;
}
...
function layoutDialog(){
 ...
 const fidelityHtml=fidelity?`<details class="dwg-fidelity-ledger"><summary>Ketelitian denah DWG <strong>${esc(readableStatus(fidelity.preservation))}</strong></summary><div class="dwg-fidelity-body"><p>Bagian ini membedakan data gambar CAD asli dan bagian yang sudah ditampilkan dalam 3D. Jumlah objek tidak selalu sama jika data gambar telah disederhanakan saat diimpor.</p><dl class="system-info-grid">${pair('Objek CAD asli',fidelity.sourceEntityCount??'UNKNOWN')+pair('Lapisan CAD asli',fidelity.sourceLayerCount??'UNKNOWN')+pair('Blok CAD asli',fidelity.sourceBlockCount??'UNKNOWN')+pair('Segmen acuan',fidelity.referenceSegmentCount)+pair('Lantai 3D',counts.floor??0)+pair('Dinding 3D',counts.walls??0)+pair('Kolom 3D',counts.columns??0)+pair('Pintu 3D',counts.doors??0)+pair('Bukaan / tirai',counts.openings??0)+pair('Label pada sumber',counts.labels??0)+pair('Pemetaan sumbu',transform.axisMap||'UNKNOWN')+pair('Faktor skala',transform.scale??'UNKNOWN')+pair('Rotasi',transform.rotation??'UNKNOWN')+pair('Titik awal X / Y',`${transform.originX??'UNKNOWN'} / ${transform.originY??'UNKNOWN'}`)}</dl><div class="dwg-unimplemented"><h4>Belum diimplementasikan sebagai 3D fisik</h4>${unimplemented.map(item=>`<div class="dwg-unimplemented-row"><strong>${esc(item.entityType)}</strong><span>${esc(item.semanticType)} · ${esc(item.threeDStatus)}</span><small>${esc(item.reason)}</small></div>`).join('')}</div></div></details>`:'';
 modal('Denah Pabrik',`<div class="card accent"><h4>${l?'DWG berhasil dimuat':'DWG belum tersedia'}</h4><p>${l?'Bentuk denah, skala, ketinggian, dan posisi diperiksa secara terpisah.':'Belum ada sumber DWG yang dapat ditampilkan.'}</p></div>${l?`<dl class="data-list">${pair('Jenis sumber',truth.source)+pair('Nama sumber',truth.sourceFile)+pair('Geometri denah',truth.planGeometry)+pair('Satuan sumber',truth.sourceUnits)+pair('Skala',truth.scale)+pair('Ketinggian',truth.elevation)+pair('Posisi '+FOUNDATION_SCOPE.referenceAssetName,positionVerification(referencePlacement))+pair('Area yang dikenali',Array.isArray(l.functionalZones)?l.functionalZones.length+' area':'UNKNOWN')}</dl>${fidelityHtml}<div class="actions"><button id="view-layout" class="primary">Buka Denah</button></div>`:''}${adminTools}`);
...
function positionStatusLabel(status){
 const labels={'CAD FOOTPRINT':'Posisi dari CAD','CAD LABEL':'Label pada CAD','CAD ZONE':'Zona pada CAD','CAD ROOM':'Ruang pada CAD','SOURCE ANNOTATION / REVIEW':'Anotasi sumber · perlu ditinjau','POSITION REVIEW REQUIRED':'Posisi perlu ditinjau'};
...
function machineDetailDialog(machine){
 ...
 const primaryData=primary?pair('ID mesin',machine.machineId)+pair('Area',machine.area)+pair('Model',machine.model)+pair('Nomor seri',machine.serial)+pair('Lokasi fungsional SAP',machine.functionalLocation)+pair('Kode SAP',machine.sapCode)+pair('Tahun',machine.year)+pair('Dasar model 3D',truth?.source3D||machine.source)+pair('Detail model 3D',truth?.detail3D||'Model berbasis referensi')+pair('Keandalan data',truth?.dataConfidence||'Sesuai sumber tersedia')+pair('Posisi',truth?.position||positionVerification(placement))+pair('Dasar posisi',positionStatusLabel(policy.positionStatus))+pair('Sumber identitas',machine.source==='USER_CONFIRMED'?'Konfirmasi pengguna':'Registry mesin'):'';
 const placeholderData=pair('ID posisi',machine.machineId)+pair('Area',machine.area)+pair('Dasar model 3D','NOT_IMPLEMENTED · LAYOUT PLACEHOLDER')+pair('Detail model 3D','NOT_IMPLEMENTED')+pair('Posisi',positionVerification(placement))+pair('Dasar posisi',positionStatusLabel(policy.positionStatus))+pair('Status detail','Belum dibuka pada fase fondasi');
...
function registryType(machine){
 ...
 if(name.includes('OFFSET'))return'Mesin cetak offset';if(name.includes('AUTOPLATEN')||name.includes('AUTOBLANKING'))return'Die Cutting / Blanking';
 ...
 if(name.includes('CTP')||name.includes('CTF')||name.includes('ZUND'))return'Prepress';if(name.includes('INKJET'))return'Cetak digital';
 if(name.includes('COLLATOR'))return'Collator';return machine.area==='UTILITY'?'Peralatan utilitas':'Peralatan produksi';
}
...
function openSceneEditor(){
 ...
 const panel=document.createElement('section');panel.id='scene-editor-panel';panel.setAttribute('aria-label','Edit pabrik 3D');panel.tabIndex=-1;document.body.append(panel);
 const editorCategories=[
  ...
  {id:'furniture',label:'Furnitur',hint:'Meja, kursi, lemari, rak, dan troli'},
...
window.addEventListener('bmj:layerchange',event=>{if(!engine)return;const {key,visible}=event.detail||{};const map={building:'building',walls:'walls',furniture:'furniture',roof:'roof',machines:'machines',labels:'labels',landscape:'landscape',reference:'reference',unidentified:'unidentified',compressedAir:'utility_compressed_air',ahuPiping:'utility_ahu_piping',ducting:'utility_ahu_ducting',utilityAnchors:'utility_anchors'};const layer=map[key];if(!layer)return;if(key==='labels'){engine.labels=Boolean(visible);$('#labels')?.classList.toggle('active',engine.labels);}engine.setFactoryLayer(layer,Boolean(visible));});
window.addEventListener('bmj:qualitychange',event=>{engine?.setQualityProfile(event.detail?.profile||'auto');});
