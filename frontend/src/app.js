...
function applyMachineShell(){
 ...
 const simCaption=$('#tool-simulation span');if(simCaption)simCaption.textContent=IS_APM2?'Proses Autoplaten':IS_SHEETING?'Proses Sheeting':IS_GENERIC?'Belum Tervalidasi':'Printing Test';
 if(IS_OFFSET10){
  ...
  const qgrid=$('[data-workbench-card="quick"] .quick-grid');if(qgrid)qgrid.innerHTML=`<dt>Model</dt><dd>${esc(m.model||'Belum tersedia')}</dd><dt>Serial</dt><dd>${esc(m.serial||'Belum tersedia')}</dd><dt>Tahun</dt><dd>${esc(m.year||'Belum tersedia')}</dd><dt>Area</dt><dd>${esc(m.area)}</dd><dt>Status geometry</dt><dd>${esc(GENERIC_CONFIG.evidence.geometry)}</dd><dt>Status simulasi</dt><dd>DIBLOKIR · belum tervalidasi</dd><dt>Catatan bukti</dt><dd>${esc(GENERIC_CONFIG.evidence.reason)}</dd>`;
