...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V65 memperbaiki main head menjadi panoramic window yang benar-benar berongga—tidak ada panel solid di belakang kaca—sehingga cylinder turquoise ber-band terlihat sebagai mekanisme di balik aperture nyata. Outfeed juga mendapat handwheel operator yang dapat dipilih, dan reference paper stack dibuat sebagai skid load tebal yang benar-benar duduk di atas pallet.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS HSM56 VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel rendah + exposed opposed chuck / fixed-position two-sided rollstand family reference · kanan')+pair('Pemotongan','Panoramic-window main head + banded process cylinder + cross-cut event · exact knife HSM-CTM7 unresolved')+pair('Output','13-belt outfeed + 3 adjustment rod assemblies + operator handwheel → rigid lift table / blue pallet tower · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V65 memprioritaskan detail yang benar-benar tampak pada brochure/foto HSM 56 2014, termasuk hollow inspection aperture, exposed reel hub, inclined guide frame, satu dominant banded process cylinder, adjustment hardware, operator handwheel dan substantial skid stack. Pencarian literal HSM-CTM7 belum menghasilkan drawing/manual/foto OEM publik yang spesifik, jadi CTM7 tidak diasumsikan berarti cut-to-mark dan exact knife tetap unresolved.</p></div>`;
  }else{
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=65`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
