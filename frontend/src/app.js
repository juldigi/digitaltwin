...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V63 mengikuti anchor yang benar-benar terlihat pada brochure HSM 56 2014: satu reel rendah dengan support dua sisi di kanan, web naik ke frame guide/tension, masuk ke main head abu-abu/toska berjendela lebar dengan cylinder proses besar di dalam, lalu melewati long narrow-belt outfeed dengan batang adjustment transversal menuju lift-table stacker berbentuk tower di kiri.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS HSM56 VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel rendah / fixed-position two-sided rollstand family reference · kanan')+pair('Pemotongan','Windowed main head + cross-cut zone · exact knife HSM-CTM7 unresolved')+pair('Output','Long narrow-belt outfeed → tower lift table / pallet · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V63 memprioritaskan bentuk yang benar-benar tampak pada brochure/foto HSM 56 2014. Listing historis HSM 52/56/65 menyebut rotary sheeting, sementara brochure BW memakai istilah flat-bed knife; karena konflik ini, cylinder besar yang terlihat di balik jendela dimodelkan sebagai process elements dan exact fungsi knife HSM-CTM7 tetap unresolved.</p></div>`;
  }else{
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=63`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
