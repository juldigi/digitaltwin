...
function openSceneEditor(){
 ...
 const overrides={...(state.sceneOverrides||{})};let selected=null,undo=[],redo=[],isolated=false;
 ...
 const apply=()=>{for(const [id,v] of baseline){const node=engine.sceneObjects.get(id);if(!node)continue;const x=overrides[id]||v;node.position.fromArray(x.position);node.rotation.set(...x.rotation);node.scale.fromArray(x.scale);node.visible=x.visible;}engine.sceneOverrides=overrides;if(isolated&&selected){let node=engine.sceneObjects.get(selected);while(node?.parent&&node!==engine.factory){for(const sibling of node.parent.children)if(sibling!==node)sibling.visible=false;node=node.parent;}}};
 const current=()=>{if(!selected)return;const node=engine.sceneObjects.get(selected);return node&&{position:node.position.toArray(),rotation:[node.rotation.x,node.rotation.y,node.rotation.z],scale:node.scale.toArray(),visible:node.visible};};
 const update=()=>{const v=current(),info=selected?engine.sceneObjectInfo(selected):null;panel.innerHTML=`<header style="display:flex;justify-content:space-between;align-items:center"><strong>Editor Scene 3D</strong><button id="se-close">Tutup</button></header><p>Pilih objek di 3D atau cari di daftar. Perubahan berlaku setelah Simpan.</p><input id="se-search" type="search" placeholder="Cari objek, dinding, mesin…" style="width:100%"><select id="se-object" size="7" style="width:100%;margin:8px 0"></select><p id="se-selected">${esc(selected||'Pilih objek')}</p><label><input id="se-snap" type="checkbox" ${engine.gizmo.translationSnap?'checked':''}> Snap posisi 10 cm / rotasi 5°</label><div class="actions"><button id="se-focus">Fokus</button><button id="se-isolate">${isolated?'Tampilkan Semua':'Isolasi sementara'}</button></div><div class="edit-strip">${['translate','rotate','scale'].map((m,i)=>`<button data-se-mode="${m}" ${engine.gizmo.mode===m?'class="active"':''}>${['Geser','Putar','Skala'][i]}</button>`).join('')}</div>${info?`<p>Ukuran (m): ${info.dimensions.map(n=>n.toFixed(2)).join(' × ')}</p>${info.collisions.length?`<p role="alert">Potensi overlap aset: ${esc(info.collisions.join(', '))}</p>`:''}`:''}<div id="se-numbers">${v?['position','rotation','scale'].map(key=>`<fieldset><legend>${key}</legend>${v[key].map((n,i)=>`<label>${'XYZ'[i]} <input data-se-key="${key}" data-se-axis="${i}" type="number" step="0.01" value="${n.toFixed(3)}" style="width:75px"></label>`).join('')}</fieldset>`).join('')+`<button id="se-hide">${v.visible?'Sembunyikan':'Tampilkan'}</button><button id="se-ground">Turunkan ke lantai</button><button id="se-reset">Reset objek</button>`:''}</div><div class="actions"><button id="se-undo">Undo</button><button id="se-redo">Redo</button><button id="se-save" class="primary">Simpan Scene</button><button id="se-history">Riwayat</button><button id="se-export">Ekspor JSON</button><label>Impor JSON<input id="se-load-file" type="file" accept="application/json,.json"></label></div><p id="se-status">${Object.keys(overrides).length} override tersimpan dalam draft.</p>`;
 const choices=[...engine.sceneObjects].map(([id,node])=>[id,node.name||node.userData?.semantic||node.parent?.name||'Objek 3D']);const fill=()=>{const query=panel.querySelector('#se-search').value.toLowerCase();panel.querySelector('#se-object').innerHTML=choices.filter(([id,name])=>(id+' '+name).toLowerCase().includes(query)).slice(0,300).map(([id,name])=>`<option value="${esc(id)}" ${id===selected?'selected':''}>${esc(name)} · ${esc(id)}</option>`).join('');};fill();panel.querySelector('#se-search').oninput=fill;panel.querySelector('#se-object').onchange=e=>engine.selectSceneObject(e.target.value);panel.querySelector('#se-snap').onchange=e=>{engine.gizmo.setTranslationSnap(e.target.checked?.1:null);engine.gizmo.setRotationSnap(e.target.checked?Math.PI/36:null);};panel.querySelector('#se-focus').onclick=()=>{if(selected)engine.fit(engine.sceneObjects.get(selected));};panel.querySelector('#se-isolate').onclick=()=>{if(!selected)return;isolated=!isolated;apply();if(isolated){let node=engine.sceneObjects.get(selected);while(node?.parent&&node!==engine.factory){for(const sibling of node.parent.children)if(sibling!==node)sibling.visible=false;node=node.parent;}}update();};
 panel.querySelectorAll('[data-se-mode]').forEach(b=>b.onclick=()=>{engine.gizmo.setMode(b.dataset.seMode);update();});panel.querySelectorAll('[data-se-key]').forEach(input=>input.onchange=()=>{const value=+input.value;if(!Number.isFinite(value)||Math.abs(value)>100000||input.dataset.seKey==='scale'&&value<=0)return toast('Angka tidak valid.',true);snapshot();const v=current();v[input.dataset.seKey][+input.dataset.seAxis]=value;overrides[selected]=v;apply();update();});
 const mutate=fn=>{if(!selected)return;snapshot();fn();overrides[selected]=current();update();};
 panel.querySelector('#se-hide')?.addEventListener('click',()=>mutate(()=>{engine.sceneObjects.get(selected).visible=!engine.sceneObjects.get(selected).visible;}));
 panel.querySelector('#se-ground')?.addEventListener('click',()=>mutate(()=>{engine.dropSceneObjectToFloor(selected);}));
 panel.querySelector('#se-reset')?.addEventListener('click',()=>{snapshot();delete overrides[selected];apply();update();});
 ...
 panel.querySelector('#se-save').onclick=async()=>{try{const next=await request('/api/scene',{method:'PUT',data:{overrides}});await acceptState(next);toast('Scene disimpan.');}catch(e){panel.querySelector('#se-status').textContent=e.message;}};
 ...
 panel.querySelector('#se-load-file').onchange=async e=>{try{const file=e.target.files?.[0];if(!file)return;if(file.size>1024*1024)throw new Error('Berkas melebihi 1 MB.');const parsed=JSON.parse(await file.text());if(parsed.schemaVersion!==1||!parsed.overrides||typeof parsed.overrides!=='object'||Array.isArray(parsed.overrides))throw new Error('Format override tidak valid.');const entries=Object.entries(parsed.overrides);if(entries.length>500||entries.some(([id,v])=>!engine.sceneObjects.has(id)||!v||typeof v.visible!=='boolean'||['position','rotation','scale'].some(key=>!Array.isArray(v[key])||v[key].length!==3||v[key].some(n=>!Number.isFinite(n)||Math.abs(n)>100000))||v.scale.some(n=>n<=0)))throw new Error('Berkas memuat ID atau transform yang tidak valid.');snapshot();Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,parsed.overrides);apply();update();}catch(error){toast(error.message,true);}};
 panel.querySelector('#se-history').onclick=async()=>{try{const history=await request('/api/scene/revisions');const id=prompt('ID revisi untuk dipulihkan:\n'+history.revisions.map(r=>r.id+' · '+r.at).join('\n'));if(!id)return;const next=await request('/api/scene/restore',{method:'PUT',data:{id}});await acceptState(next);Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,next.sceneOverrides||{});apply();update();}catch(e){toast(e.message,true);}};
 };engine.onSceneSelect=(id)=>{selected=id;update();};const onDragStart=()=>{if(selected)snapshot();};const onDragEnd=()=>{if(selected){overrides[selected]=current();update();}};engine.gizmo.addEventListener('mouseDown',onDragStart);engine.gizmo.addEventListener('mouseUp',onDragEnd);engine.onSceneTransform=()=>{if(selected)overrides[selected]=current();};update();
}
