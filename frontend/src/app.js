...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V67 mempertahankan true inspection aperture dan detail V65, lalu memecah delivery menjadi tiga zona proses yang lebih masuk akal: fast tape untuk membuka gap sesudah cut, slow tape untuk transfer, lalu overlap tape sebelum stacker. Geometry exact HSM-CTM7 tetap tidak diklaim bila sumber spesifik tidak tersedia.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS HSM56 VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel rendah + exposed opposed chuck / fixed-position two-sided rollstand family reference · kanan')+pair('Pemotongan','Panoramic-window main head + banded process cylinder + visible flat-bed knife family reference · exact HSM-CTM7 actuation unresolved')+pair('Output','13 jalur tape × 3 zona proses (fast / slow / overlap) + adjustment assemblies + operator handwheel → lift-table stacker · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V67 tetap memakai foto resmi HSM 56 untuk bentuk, tetapi logic proses simulation sekarang dipisahkan dari geometry evidence. Pasaban dipakai hanya untuk urutan fast belt → slow belt → overlay → stacking, Unico untuk hubungan web/draw-roll/cutter/tapes, dan Maxson untuk logika overlap/lift table. Tidak ada sumber sekunder tersebut yang dipakai untuk mengklaim bentuk exact HSM-CTM7.</p></div>`;
  }else{
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=67`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
