...
let state,engine,activeTab='overview',apiBase='',token='',role=null,editing=false,explode=0,selectedPart=null,selectedTaxonomyId=ACTIVE_ROOT,exteriorMode=false,exteriorPreviousLow=null,exteriorFocusKey=null,simulationState,simulationOwnsExterior=false,referenceCategoryFilter='all',toastTimer,bundledLayout=null,cachedDataActive=false,factoryAssetContextId=null;
...
function renderFactoryPanel(){
 ...
 const ahuIds=new Set(['BMJ-MCH-0036','BMJ-MCH-0037','BMJ-MCH-0038','BMJ-MCH-0039','BMJ-MCH-0040','BMJ-MCH-0041']),ahuPlacements=layout.placements.filter(p=>ahuIds.has(p.machineId)),knownAhu=ahuPlacements.filter(p=>p.status!=='UNIDENTIFIED');
 const compressorIds=new Set(['BMJ-MCH-0029','BMJ-MCH-0030','BMJ-MCH-0031','BMJ-MCH-0032','BMJ-MCH-0033','BMJ-MCH-0034','BMJ-MCH-0035']),compressorPlacements=layout.placements.filter(p=>compressorIds.has(p.machineId)),knownCompressors=compressorPlacements.filter(p=>p.status!=='UNIDENTIFIED');
 const routing=engine?.actualFactory?.utilityRouting||engine?.actualFactory?.root?.userData?.utilityRouting;
 const routingSystems=routing?.systems||[];
 $('#panel-content').innerHTML=`<h3>Bangunan & mesin</h3><p class="subtle">${known.length} mesin terpetakan · ${unknown.length} menunggu identifikasi posisi.</p><div class="card">${[['roof','Atap'],['building','Dinding & ruangan'],['machines','Mesin terpetakan'],['labels','Nama area & mesin'],['landscape','Taman & akses luar'],['unidentified','Area belum teridentifikasi'],['reference','Garis denah sumber'],['utility_compressed_air',(routing?.actualRoutingApplied?'Aktual':'Acuan')+' · Pipa compressor'],['utility_ahu_piping',(routing?.actualRoutingApplied?'Aktual':'Acuan')+' · Pipa AHU'],['utility_ahu_ducting',(routing?.actualRoutingApplied?'Aktual':'Acuan')+' · Ducting AHU'],['utility_anchors','Titik sambungan utilitas']].map(([key,text])=>`<label class="check"><input type="checkbox" data-factory-layer="${key}" ${layers[key].visible?'checked':''}> ${text}</label>`).join('')}</div><label for="factory-asset-focus">Cari posisi mesin</label><select id="factory-asset-focus"><option value="">Pilih mesin</option>${layout.placements.map(p=>`<option value="${p.machineId}">${esc(p.label)}${p.status==='UNIDENTIFIED'?' · belum teridentifikasi':''}</option>`).join('')}</select><div class="actions"><button id="factory-overview" class="secondary">Lihat seluruh pabrik</button><button id="factory-unknown" class="secondary">Area belum teridentifikasi</button></div><div class="card accent"><h4>Jalur utilitas siap dilengkapi</h4><p>${routing?(routing.mode==='TEMPLATE_ONLY'?'Jalur acuan untuk pipa compressor, pipa AHU, dan ducting AHU sudah tersedia. Jalur ini disembunyikan saat awal dibuka agar tidak tertukar dengan kondisi aktual. Saat gambar routing aktual tersedia, titik dan jalurnya dapat disesuaikan tanpa membangun ulang model.':`Data routing sudah tersedia dalam status ${routing.mode}. Bagian yang belum terverifikasi tetap ditampilkan sebagai acuan sampai data lapangan lengkap tersedia.`):'Data jalur utilitas belum tersedia.'}</p><p class="subtle">${routingSystems.map(x=>`${x.system}: ${x.nodeCount} titik · ${x.segmentCount} jalur · ${x.equipmentAnchorCount} titik peralatan`).join(' · ')}</p><div class="actions"><button class="secondary" data-routing-focus="utility_compressed_air">Lihat pipa compressor</button><button class="secondary" data-routing-focus="utility_ahu_piping">Lihat pipa AHU</button><button class="secondary" data-routing-focus="utility_ahu_ducting">Lihat ducting AHU</button></div></div><div class="card"><h4>AHU & ducting</h4><p>${knownAhu.length===ahuPlacements.length?'Seluruh titik acuan AHU sudah tersedia. Jalur supply, return, udara luar, dan pipa utilitas sudah siap sebagai acuan; gambar aktual nantinya cukup memperbarui titik, elevasi, ukuran, dan fitting.':`${knownAhu.length} dari ${ahuPlacements.length} AHU memiliki titik acuan yang tervalidasi. Pipa AHU dan ducting sudah tersedia pada lapisan terpisah, tetapi belum diposisikan sebagai kondisi aktual sampai lokasi dan gambar routing tersedia.`}</p></div><div class="card"><h4>Udara bertekanan & pipa</h4><p>${knownCompressors.length===compressorPlacements.length?'Seluruh titik acuan compressor sudah tersedia. Jalur discharge, header, ring main, dan cabang servis siap disesuaikan dengan routing aktual.':`${knownCompressors.length} dari ${compressorPlacements.length} compressor memiliki titik acuan yang tervalidasi. Jalur discharge → treatment → ring main → cabang servis sudah tersedia sebagai acuan, tetapi belum dianggap sebagai kondisi aktual lapangan.`}</p></div><div class="card"><h4>Ketelitian tampilan</h4><p>Denah 250804 mengikuti revisi posisi Anda. Tinggi bangunan, bentuk atap, detail pintu, taman, tanaman, dan perlengkapan ruang merupakan perkiraan visual. Posisi berbasis label masih berupa zona, bukan titik ukur. Arah feeder–delivery perlu verifikasi.</p></div>`;
 const canonicalFactoryLayer={roof:'roof',building:'building',machines:'machines',labels:'labels',landscape:'landscape',unidentified:'unidentified',reference:'reference',utility_compressed_air:'compressedAir',utility_ahu_piping:'ahuPiping',utility_ahu_ducting:'ducting',utility_anchors:'utilityAnchors'};
 ...
 $('#factory-asset-focus').onchange=e=>{if(e.target.value){const p=layout.placements.find(p=>p.machineId===e.target.value),m=MACHINE_REGISTRY_BY_ID.get(e.target.value);engine.setFactoryLayer(p.status==='UNIDENTIFIED'?'unidentified':'machines',true);if(m&&isFoundationPrimary(m))openPrimaryFactoryContext(m,{historyMode:'push'});else engine.focusFactoryAsset(e.target.value);}};
 $$('[data-routing-focus]').forEach(button=>button.onclick=()=>{const key=button.dataset.routingFocus;engine.setFactoryLayer(key,true);const input=document.querySelector(`[data-factory-layer="${key}"]`);if(input)input.checked=true;const canonical=canonicalFactoryLayer[key];if(canonical)emitDomainState({visibleLayers:{[canonical]:true}});engine.fit(layers[key]);});
 $('#factory-overview').onclick=()=>{factoryAssetContextId=null;renderPanel('overview');engine.fitObjects?.([engine.factory,engine.machine?.visible?engine.machine:null]);};$('#factory-unknown').onclick=()=>{factoryAssetContextId=null;engine.setFactoryLayer('unidentified',true);engine.fit(layers.unidentified);};return true;
}
...
function buildUniversalSearchIndex(){
 ...
 for(const machine of MACHINE_REGISTRY){
  ...
  items.push({type:'machine',group:'MESIN',title:machine.name,subtitle:[machine.area,machine.sapCode,machine.model].filter(Boolean).join(' · '),route,machineId:machine.machineId,has3D:scopedRegistryHas3D(machine),keywords:[machine.name,machine.machineId,machine.sapCode,machine.functionalLocation,machine.model,machine.serial,machine.area].filter(Boolean).join(' ')});
 ...
 for(const system of [
  ...
  {title:'Udara Bertekanan',key:'compressedAir',keywords:'compressor compressed air udara bertekanan piping pipa distribusi ring main'},
  {title:'Air / IPAL',key:'water',keywords:'water air ipal wastewater treatment pengolahan'},
  {title:'Kelistrikan',key:'electrical',keywords:'electrical listrik power kelistrikan'},
  {title:'Jalur Utilitas',key:'routing',keywords:'utility routing jalur utilitas piping pipa ducting'}
...
async function openAssetContext(machine){
 ...
 openPrimaryFactoryContext(machine,{historyMode:'push'});
}
...
try{engine=new FactoryEngine($('#viewport'),part=>{const meta=taxonomyForPart(part);if(meta)selectedTaxonomyId=meta.id;choosePart(part);showPanel();renderPanel('structure');});engine.onFactoryPartSelect=part=>{if(!factoryAssetContextId)return false;const meta=taxonomyForPart(part);if(meta)selectedTaxonomyId=meta.id;choosePart(part);showPanel();renderPanel('structure');return true;};engine.onTaxonomySelect=id=>selectTaxonomy(id,{revealPanel:false});engine.onSimulationUpdate=next=>{simulationState=next;updateSimulationPanel(next);};simulationState=engine.getPrintingSimulationState();engine.onReset=()=>{explode=0;selectedPart=null;selectedTaxonomyId=ACTIVE_ROOT;renderPanel();};engine.onError=message=>toast(message,true);$('#engine-status').textContent='Menyiapkan denah pabrik';try{engine.setLow(localStorage.getItem('offset5-low')==='1'||matchMedia('(max-width:767px)').matches||matchMedia('(pointer:coarse) and (max-width:1024px)').matches);}catch{}}catch(e){renderStaticMachineFallback(e);$('#engine-status').textContent='Tampilan 3D belum tersedia';}
try{
 bundledLayout=await loadBundledPlantLayout();bundledLayout.fleet=await loadFactoryFleet();engine?.loadLayout(activeLayout());
 if(engine)engine.onFactorySelect=id=>{const m=MACHINE_REGISTRY.find(m=>m.machineId===id);if(!m)return;if(isFoundationPrimary(m))openPrimaryFactoryContext(m,{historyMode:'push'});else machineDetailDialog(m);};
 renderStatus();redrawPlantPlan();
 const deepNode=INITIAL_URL_STATE.get('node'),initialPlaceholder=INITIAL_REQUESTED_ASSET&&!canOpenTechnical3D(INITIAL_REQUESTED_ASSET)?machineRecordForRoute(INITIAL_REQUESTED_ASSET):null,initialPrimary=!INITIAL_URL_STATE.get('machine')&&INITIAL_REQUESTED_ASSET&&canOpenTechnical3D(INITIAL_REQUESTED_ASSET)?machineRecordForRoute(INITIAL_REQUESTED_ASSET):null;
 if(engine){
