...
function renderPanel(tab=activeInspectorTab()){
 ...
 if(tab==='overview'){
  ...
  $('#explode').addEventListener('input',e=>{setExplodeLevel(+e.target.value);engine?.template.explode(explodeLevel(),selectedPart);if(selectedPart)engine.template.ghost(explodeLevel()>0,selectedPart);$('#explode-value').textContent=Math.round(explodeLevel()*100)+'%';$('#ghost').classList.toggle('active',!!engine?.template.ghosted);$('#tool-explode')?.classList.toggle('active',explodeLevel()>0);emitDomainState({inspectionMode:{explode:explodeLevel()>0}});});
...
on('#tool-explode',()=>{if(!ensureMachineInspectionContext('Urai')||simulationLocksStructure())return;showPanel();renderPanel('structure');setExplodeLevel(explodeLevel()>.01?0:.65);engine?.template.explode(explodeLevel(),selectedPart);$('#tool-explode')?.classList.toggle('active',explodeLevel()>0);emitDomainState({inspectionMode:{explode:explodeLevel()>0}});renderPanel('structure');});
