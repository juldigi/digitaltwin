...
const MACHINE_KEY=['offset10','apm2'].includes(REQUESTED_MACHINE)||GENERIC_CONFIG?REQUESTED_MACHINE:'offset5';
const IS_OFFSET10=MACHINE_KEY==='offset10',IS_APM2=MACHINE_KEY==='apm2',IS_GENERIC=!!GENERIC_CONFIG;
const GENERIC_TAXONOMY=IS_GENERIC?universalTaxonomy(MACHINE_KEY):[],GENERIC_ROOT=GENERIC_TAXONOMY[0]?.id;
const ACTIVE_ROOT=IS_OFFSET10?'O10':IS_APM2?'APM2':IS_GENERIC?GENERIC_ROOT:'O5';
const ACTIVE_TAXONOMY=IS_OFFSET10?OFFSET10_TAXONOMY:IS_APM2?APM2_TAXONOMY:IS_GENERIC?GENERIC_TAXONOMY:OFFSET5_TAXONOMY;
const TAXONOMY_BY_ID=IS_OFFSET10?OFFSET10_TAXONOMY_BY_ID:IS_APM2?APM2_TAXONOMY_BY_ID:IS_GENERIC?new Map(GENERIC_TAXONOMY.map(n=>[n.id,n])):OFFSET5_BY_ID;
const taxonomyChildren=IS_OFFSET10?offset10TaxonomyChildren:IS_APM2?apm2TaxonomyChildren:IS_GENERIC?(id=>GENERIC_TAXONOMY.filter(n=>n.parentId===id)):offset5Children;
const taxonomyStats=IS_OFFSET10?offset10TaxonomyStats:IS_APM2?apm2TaxonomyStats:IS_GENERIC?(()=>({total:GENERIC_TAXONOMY.length,byLevel:Object.fromEntries([1,2,3,4,5,6].map(l=>[l,GENERIC_TAXONOMY.filter(n=>n.level===l).length]))})):offset5Stats;
const PHOTO_REGISTRY=IS_OFFSET10?OFFSET10_PHOTO_REGISTRY:IS_APM2?APM2_PHOTO_REGISTRY:IS_GENERIC?[]:OFFSET5_PHOTOS;
...
const TECHNICAL_SOURCES=IS_OFFSET10?OFFSET10_TECHNICAL_SOURCES:IS_APM2?APM2_TECHNICAL_SOURCES:IS_GENERIC?GENERIC_SOURCES:OFFSET5_SOURCES;
const photoStats=IS_OFFSET10?offset10PhotoStats:IS_APM2?apm2PhotoStats:IS_GENERIC?(()=>({unique:0,total:0})):offset5PhotoStats;
const ORIENTATION=IS_OFFSET10?OFFSET10_ORIENTATION:IS_APM2?APM2_ORIENTATION:IS_GENERIC?{feedDirection:'Mengikuti alur proses keluarga',operatorSide:'Belum terverifikasi',driveSide:'Belum terverifikasi'}:OFFSET5_ORIENTATION;
const PRINTING_SIMULATION_STAGES=IS_OFFSET10?OFFSET10_SIMULATION_STAGES:IS_APM2?APM2_SIMULATION_STAGES:IS_GENERIC?GENERIC_CONFIG.modules:OFFSET5_SIM_STAGES;
const INK_SIMULATION_SEQUENCE=IS_OFFSET10?OFFSET10_INK_SEQUENCE:IS_APM2?APM2_PROCESS_STEPS:IS_GENERIC?GENERIC_CONFIG.modules.map(x=>`${x} process`):OFFSET5_INK_SEQUENCE;
