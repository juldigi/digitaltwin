...
function openSceneEditor(){
 ...
 <div class="se-progress" aria-label="Alur editor"><span class="done"><b>1</b>Pilih objek</span><span class="${selected?'done':'current'}"><b>2</b>Atur</span><span class="${draftCount?'current':''}"><b>3</b>Simpan</span></div>
 ...
 <section class="se-step ${selected?'':'is-disabled'}">
  ...
  <div class="se-quick-row"><button id="se-isolate">${isolated?'Tampilkan semua':'Fokus hanya objek ini'}</button><label class="se-snap-toggle"><input id="se-snap" type="checkbox" ${engine.gizmo.translationSnap?'checked':''}><span>Gerak presisi</span></label></div>
  <div class="se-nudge-card"><strong>Geser sedikit</strong><span>Gunakan tombol ini jika sulit menggeser lewat 3D.</span><div class="se-nudge" aria-label="Gerakkan objek"><button data-se-nudge="x:-1">← Kiri</button><button data-se-nudge="x:1">Kanan →</button><button data-se-nudge="z:-1">↓ Depan</button><button data-se-nudge="z:1">Belakang ↑</button><button data-se-nudge="y:-1">Turun</button><button data-se-nudge="y:1">Naik</button><button data-se-turn="-1">↶ Putar kiri</button><button data-se-turn="1">Putar kanan ↷</button></div></div>
  ...
  ${info?`<div class="se-info-line"><span>Ukuran</span><strong>${info.dimensions.map(n=>n.toFixed(2)).join(' × ')} m</strong></div>`:''}
  ${info?.collisions.length?`<div class="se-alert warning"><strong>Objek kemungkinan bertabrakan</strong><span>Periksa: ${esc(info.collisions.join(', '))}</span></div>`:''}
  ...
  <details class="se-advanced"><summary>Pengaturan lebih detail</summary>
   <div class="se-advanced-body">
    ...
    <div id="se-numbers">${v?['position','rotation','scale'].map(key=>`<fieldset><legend>${key==='position'?'Posisi':key==='rotation'?'Rotasi':'Ukuran / skala'}</legend>${v[key].map((n,i)=>`<label>${'XYZ'[i]} <input data-se-key="${key}" data-se-axis="${i}" type="number" step="0.01" value="${n.toFixed(3)}" ${v.locked?'disabled':''}></label>`).join('')}</fieldset>`).join(''):''}</div>
    ...
    <small class="se-technical-id">ID teknis: ${esc(selected)}</small>
   </div>
  </details>`:''}
 </section>
 ...
 <details class="se-admin-tools"><summary>Alat lanjutan Superadmin</summary><div class="se-advanced-body">
  ...
  <div class="se-inline-actions"><button id="se-history">Riwayat perubahan</button><button id="se-export">Ekspor data scene</button></div>
 ...
 const choices=[...engine.sceneObjects].filter(([id,node])=>!(id.startsWith('node:')&&node.userData.editorStableId)&&!(id.startsWith('machine:')&&node.userData.nodeId&&engine.sceneObjects.has('part:'+engine.machineKey+':'+node.userData.nodeId))).filter(([id])=>editorScope==='machine'?(id.startsWith('machine:'+engine.machineKey+':')||id.startsWith('part:'+engine.machineKey+':')):!id.startsWith('machine:')&&!id.startsWith('part:')).map(([id,node])=>{const lineage=[];for(let parent=node.parent;parent&&lineage.length<2;parent=parent.parent)if(parent.name)lineage.unshift(parent.name);return [id,[...lineage,node.name||node.userData?.semantic||'Objek 3D'].join(' / ')];});const fill=()=>{const query=panel.querySelector('#se-search').value.toLowerCase();panel.querySelector('#se-object').innerHTML=choices.filter(([id,name])=>(id+' '+name).toLowerCase().includes(query)).slice(0,300).map(([id,name])=>`<option value="${esc(id)}" ${id===selected?'selected':''}>${esc(name)}</option>`).join('');};panel.querySelector('#se-search').value=queryBefore;fill();panel.querySelector('#se-search').oninput=fill;panel.querySelector('#se-object').onchange=e=>engine.selectSceneObject(e.target.value);panel.querySelector('#se-snap').onchange=e=>{engine.gizmo.setTranslationSnap(e.target.checked?.1:null);engine.gizmo.setRotationSnap(e.target.checked?Math.PI/36:null);};panel.querySelector('#se-focus').onclick=()=>{if(selected)engine.fit(engine.sceneObjects.get(selected));};panel.querySelector('#se-isolate').onclick=()=>{if(!selected)return;isolated=!isolated;apply();update();};
