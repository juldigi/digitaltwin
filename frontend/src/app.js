...
function openSceneEditor(){
 ...
 panel.querySelector('#se-close').onclick=()=>{if(JSON.stringify(overrides)!==JSON.stringify(state.sceneOverrides||{})&&!confirm('Perubahan yang belum disimpan akan dibuang. Tutup Editor?'))return;isolated=false;isolationGuard.restore();Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,state.sceneOverrides||{});engine.applySceneOverrides(state.sceneOverrides||{});engine.gizmo.setTranslationSnap(null);engine.gizmo.setRotationSnap(null);engine.setSceneEditing(false);engine.onSceneSelect=null;engine.onSceneTransform=null;engine.gizmo.removeEventListener('mouseDown',onDragStart);engine.gizmo.removeEventListener('mouseUp',onDragEnd);panel.remove();};
