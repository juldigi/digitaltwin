...
async function switchActiveMachine(route,{historyMode='push'}={}){
 ...
 if(normalizedRoute===MACHINE_KEY){
  ...
  emitDomainState({selectedAsset:FOUNDATION_SCOPE.primaryRoute,selectedNode:null,selectedSystem:null,activeReference:null,activeSection:'asset',sceneMode:'machine',cameraPreset:'iso',inspectionMode:{explode:false,isolate:false,section:false,interior:false},inspectorState:{open:true,tab:'overview'}});return true;
 }
...
on('#tool-simulation',async()=>{if(engine?.view!=='machine')await switchActiveMachine(FOUNDATION_SCOPE.primaryRoute,{historyMode:'push'});emitDomainState({selectedSystem:null,activeReference:null,activeSection:'simulation',sceneMode:'machine'});showPanel();renderPanel('simulation');});
