...
if(IS_SHEETING)state.asset={...state.asset,asset_id:'BMJ-MCH-0002',asset_code:'SBM-2',codename:'SBM-2',model:'HSM-CTM7',description:'SHEETING LEXUS',source_description:'BMJ Machine Database',manufacturer:'LEXUS',specification:'HSM-CTM7 · Lexus Indonesia + Great Wall/Accura visual family · 2014',configuration:'Double shaftless unwind ref. → Open Feed/Tension/EPC → Compact Cross-Cut Cutter → Overlap/Layboy → Stacker','3d_status':'DEDICATED PROCEDURAL / SOURCE-GROUNDED VISUAL RECONSTRUCTION',data_confidence:'IDENTITY VERIFIED / VISUAL FAMILY REFERENCE / CUTTER TYPE UNRESOLVED',discovery_status:'LEXUS_GREATWALL_FAMILY_REFERENCES_AVAILABLE',serial_number:'00982',functional_location:'PC-PK2-OFS-SHT-SHEETING01',year:2014,sources:SHEETING_TECHNICAL_SOURCES};
...
function applyMachineShell(){
 ...
 if(IS_OFFSET10){
  ...
  const qgrid=$('[data-workbench-card="quick"] .quick-grid');if(qgrid)qgrid.innerHTML='<dt>Model plant</dt><dd>HSM-CTM7</dd><dt>Serial</dt><dd>00982</dd><dt>Tahun</dt><dd>2014</dd><dt>SAP Code</dt><dd>SBM-2</dd><dt>Referensi visual</dt><dd>Lexus Indonesia + Great Wall / Accura</dd><dt>HSM 56 pembanding</dt><dd>2014 · 300 m/min · 600 gsm</dd><dt>Jenis cutter exact</dt><dd>Belum terverifikasi</dd><dt>Alur aktual</dt><dd>RIGHT → LEFT</dd>';
...
const EXTERIOR_AREAS=IS_APM2?[
 ...
 {key:'rollstand',name:'Double Shaftless Unwind',ids:['sheeting-rollstand']},
 {key:'feed',name:'Open Feed / Tension / EPC',ids:['sheeting-feed']},
 {key:'cutter',name:'Compact Cross-Cut Cutter',ids:['sheeting-cutter','sheeting-knife','sheeting-cutter-transport']},
 {key:'delivery',name:'Overlap / Delivery / Layboy',ids:['sheeting-delivery','sheeting-overlap','sheeting-layboy']},
...
function updateSimulationPanel(next=simulationState){
 ...
 if(start)start.textContent=active?(running?'Running':'Lanjutkan'):(IS_APM2?'Mulai Simulasi Proses':IS_SHEETING?'Mulai Simulasi Sheeting':'Mulai Printing Test');
...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V64 mengikuti anchor komponen dari foto resmi HSM 56: reel rendah dengan exposed chuck/hub dan swing-hydraulic support di kanan; web naik melalui frame guide miring empat roller; main head abu-abu/toska memakai panoramic window dengan satu cylinder turquoise ber-band terang; outfeed memakai belt longitudinal dengan tiga adjustment rod, triangular support, collar dan knob; output berakhir pada rigid lift-table stacker tower di kiri.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS HSM56 VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel rendah + exposed opposed chuck / fixed-position two-sided rollstand family reference · kanan')+pair('Pemotongan','Panoramic-window main head + banded process cylinder + cross-cut event · exact knife HSM-CTM7 unresolved')+pair('Output','13-belt outfeed + 3 adjustment rod assemblies → rigid lift table / pallet tower · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V64 memprioritaskan detail yang benar-benar tampak pada brochure/foto HSM 56 2014, termasuk exposed reel hub, inclined guide frame, satu dominant banded process cylinder, adjustment hardware dan stacker tower. Listing historis HSM 52/56/65 menyebut rotary sheeting sementara brochure BW memakai istilah flat-bed knife; exact fungsi knife HSM-CTM7 tetap unresolved.</p></div>`;
  }else{
  ...
  if(IS_APM2){
   ...
   $('#panel-content').innerHTML=`<h3>Simulasi Proses Sheeting</h3><p class="subtle">Simulasi mengikuti orientasi mesin yang sudah dikoreksi: paper reel masuk dari <b>kanan</b> melalui double shaftless unwind reference, web bergerak ke <b>kiri</b> melalui open feed/tension/EPC, masuk ke compact cross-cut cutter, diteruskan melalui tape/overlap delivery, lalu ditumpuk pada layboy / lift table di sisi kiri. Kecepatan dan timing animasi bersifat visual, bukan setelan servis OEM.</p><div class="card accent simulation-overview"><div class="simulation-status-row"><span id="sim-status" class="simulation-state">${s.running?'RUNNING':s.active?'PAUSED':'READY'}</span><b id="sim-stage">${esc(s.stage||'Double Shaftless Unwind')}</b></div><div class="simulation-progress"><span id="sim-progress-bar" style="width:${Math.round((s.progress||0)*100)}%"></span></div><dl class="data-list compact"><dt>Lembar selesai</dt><dd id="sim-completed">${s.completed||0}</dd><dt>Lembar bergerak</dt><dd id="sim-visible">${s.sheetsVisible||0}</dd><dt>Lembar di pile</dt><dd id="sim-pile">${s.pileSheetsVisible||0}</dd>${pair('Mekanisme aktif',s.mechanismCount||0)+pair('Roll / drive aktif',s.rotorCount||0)}</dl><div class="actions simulation-actions"><button id="sim-start" class="primary">${s.active?(s.running?'Running':'Lanjutkan'):'Mulai Simulasi Sheeting'}</button><button id="sim-pause" class="secondary">${s.running?'Pause':'Lanjut'}</button><button id="sim-stop" class="secondary">Stop & Reset</button></div></div><h4>Kecepatan visual</h4><div class="simulation-speed">${[.5,1,1.5,2].map(v=>`<button data-sim-speed="${v}" class="${Math.abs((s.speed||1)-v)<.01?'active':''}">${v}×</button>`).join('')}</div><label class="check"><input id="sim-path" type="checkbox" ${s.pathVisible!==false?'checked':''}> Tampilkan jalur web / sheet</label><h4>Alur proses</h4><div class="simulation-ink-flow">${SHEETING_PROCESS_STEPS.map((step,index)=>`<div><span>${index+1}</span><b>${esc(step)}</b></div>`).join('')}</div><div class="simulation-flow">${SHEETING_SIMULATION_STAGES.map(stage=>`<div data-sim-stage="${esc(stage)}" class="${stage===s.stage?'active':''}"><span></span><b>${esc(stage)}</b></div>`).join('')}</div><div class="card"><h4>Gerak yang divisualisasikan</h4><p>Paper reel/chuck dan guide/tension rollers bergerak, cross-cut mechanism melakukan siklus potong, cut sheet bergerak melalui tape/overlap delivery, dan pile bertambah pada lift table. Jalur material selalu RIGHT → LEFT sesuai koreksi orientasi sebelumnya.</p></div>`;
   on('#sim-start',startPrintingSimulation);on('#sim-pause',pausePrintingSimulation);on('#sim-stop',()=>stopPrintingSimulation({restoreExterior:true}));
   document.querySelectorAll('[data-sim-speed]').forEach(b=>b.onclick=()=>{simulationState=engine?.setPrintingSimulationSpeed(+b.dataset.simSpeed)||simulationState;updateSimulationPanel(simulationState);});
   const pathToggle=$('#sim-path');if(pathToggle)pathToggle.onchange=e=>{simulationState=engine?.setPrintingSimulationPathVisible(e.target.checked)||simulationState;updateSimulationPanel(simulationState);};
   updateSimulationPanel(s);
  ...
  const ps=photoStats();
  $('#panel-content').innerHTML=`<h3>Referensi</h3><div class="card accent"><h4>${IS_OFFSET10||IS_APM2||IS_SHEETING?TECHNICAL_SOURCES.length+' referensi teknis':ps.unique+' foto mesin tersedia'}</h4><p>${IS_OFFSET10?'Model Offset 10 dibangun dari final drawing, proposal, layout UV, pre-installation, system diagram, technical data final, serta referensi resmi Heidelberg CX 104 dan FoilStar. Tidak ada foto aktual Offset 10 yang digunakan.':IS_APM2?'Identitas APM 2 berasal dari database BMJ. Geometry dan fungsi memakai referensi legacy BOBST SP 102 se-era karena foto aktual dan suffix varian mesin belum tersedia.':IS_SHEETING?'Identitas HSM-CTM7, serial 00982, SAP SBM-2 dan tahun 2014 berasal dari database BMJ. Geometry V64 memakai photo-anchor HSM 56 2014 sampai level komponen—low reel/chuck, swing-hydraulic support, inclined guide rollers, panoramic window, banded process cylinder, guide fingers, service plate, belt/rod/collar/knob outfeed, low console, stacker guard/cabinet/lift/pallet—sementara detail exact HSM-CTM7 tetap unresolved jika tidak didukung sumber spesifik.':'Foto digunakan untuk membantu menyusun bentuk luar dan orientasi mesin. Dokumen mesin digunakan untuk membantu mengenali nama dan fungsi bagian.'}</p><span class="tag">${IS_OFFSET10?'DOKUMEN PROYEK':IS_APM2?'DATABASE BMJ':IS_SHEETING?'DATABASE + FAMILY REF':'FOTO MESIN'}</span><span class="tag">DOKUMEN TEKNIS</span></div>${PHOTO_REGISTRY.length?'<h3>Foto aktual</h3>':''}${PHOTO_REGISTRY.map(p=>`<div class="card source-photo"><h4>${esc(p.filename)}</h4><p>${esc(p.machineZone)} · ${esc(p.viewDirection)}</p></div>`).join('')}<h3>Dokumen mesin</h3>${TECHNICAL_SOURCES.map(src=>`<div class="card"><h4>${esc(src.title)}</h4><p>${esc(src.publisher)}</p>${src.url?`<p><a href="${esc(src.url)}" target="_blank" rel="noopener">Buka dokumen ↗</a></p>`:''}</div>`).join('')}<div class="card"><h4>Arah mesin</h4><p>${esc(ORIENTATION.feedDirection)} · operator side ${esc(ORIENTATION.operatorSide)} · drive side ${esc(ORIENTATION.driveSide)}</p></div><p class="subtle">Jika data ukuran belum tersedia, aplikasi menampilkannya sebagai belum tersedia.</p>`;
 }
}
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=64`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
