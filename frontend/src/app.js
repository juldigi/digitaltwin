...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V59 mengikuti sumber visual Lexus Indonesia + Great Wall/Accura: paper reel berada di sisi kanan pada double shaftless unwind reference, web naik melalui open feed/tension/EPC menuju compact cross-cut cutter, lalu cut sheet diteruskan melalui overlap delivery ke layboy di sisi kiri.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS / GREAT WALL VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel / double hydraulic shaftless unwind reference · kanan')+pair('Pemotongan','Compact cross-cut cutter · jenis exact belum terverifikasi')+pair('Output','Overlap delivery → layboy / lift table · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V59 memakai referensi Lexus Indonesia dan visual Great Wall/Accura untuk arsitektur open-frame; HSM 56 tetap menjadi pembanding. Karena sumber publik berbeda mengenai knife dan rollstand, jenis cutter exact HSM-CTM7 tidak diklaim sebagai verified.</p></div>`;
  }else{
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=58`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
