...
const EXTERIOR_AREAS=IS_APM2?[
 ...
 {key:'rollstand',name:'Rollstand / Unwind',ids:['sheeting-rollstand']},
 {key:'feed',name:'Feed / Tension / EPC',ids:['sheeting-feed']},
 {key:'cutter',name:'Flat-Bed Knife / Cutter',ids:['sheeting-cutter','sheeting-knife','sheeting-cutter-transport']},
 {key:'delivery',name:'Delivery / Layboy / Stacker',ids:['sheeting-delivery','sheeting-layboy']},
...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Model mempertahankan baseline visual yang sudah dikonfirmasi: paper reel berada di sisi kanan, web bergerak ke kiri melalui feed/tension/EPC dan flat-bed knife, lalu cut sheet diteruskan ke delivery/layboy dan menumpuk di sisi kiri.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">HSM 56 FAMILY REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel / fixed-position rollstand · kanan')+pair('Pemotongan','Flat-bed knife / counter-knife')+pair('Output','Layboy / flat-plate lift table · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Data HSM 56 digunakan sebagai referensi keluarga untuk arsitektur proses; dimensi visual 3D bukan CAD engineering dan tidak menyatakan HSM-CTM7 identik dengan HSM 56.</p></div>`;
  }else{
