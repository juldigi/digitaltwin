...
function updateLODStatus(next=engine?.getLODState?.()){
 const el=$('#lod-status');if(!el)return;
 if(!next){el.textContent='LOD · belum tersedia';el.removeAttribute('title');return;}
 el.textContent=next.activeLabel;
 el.title=next.preparedLabel?`${next.preparedLabel} disiapkan sebagai konteks arsitektur; Phase-1 hanya mengaktifkan LOD 0–1.`:'Phase-1 mengaktifkan LOD 0 untuk pabrik dan LOD 1 untuk OFFSET 5.';
}
...
function choosePart(part){if(!part||!engine||simulationLocksStructure())return;engine.template.reset();engine.isolated=false;explode=0;selectedPart=part;engine.template.highlight(part);engine.template.ghost(true,part);engine.setPartLabels(part,selectedTaxonomyId);engine.fit(part);const meta=taxonomyForPart(part);engine.setPreparedLODHint(meta?.level||1);emitDomainState({selectedAsset:MACHINE_KEY,selectedNode:meta?.id||part.userData?.nodeId||selectedTaxonomyId,inspectorState:{open:true,tab:'structure'}});}
...
function resetTaxonomyRoot(){
 ...
 if(engine){engine.template.reset();engine.clearPartLabels();engine.isolated=false;engine.clearPreparedLODHint();engine.fit(engine.machine);}
...
function selectTaxonomy(id,{revealPanel=false}={}){
 ...
 if(part)choosePart(part);else{selectedPart=null;engine?.template.reset();engine?.clearPartLabels();engine?.setPreparedLODHint(meta.level);emitDomainState({selectedAsset:MACHINE_KEY,selectedNode:meta.id,inspectorState:{open:true,tab:'structure'}});}
...
function machineDetailDialog(machine){
 ...
 const primaryData=pair('Nama aset',state?.asset?.description||machine.name)+pair('Machine ID',machine.machineId)+pair('Model',state?.asset?.model||machine.model)+pair('Kategori',state?.asset?.category||'UNKNOWN')+pair('Subkategori',state?.asset?.subcategory||'UNKNOWN')+pair('Pabrikan',state?.asset?.manufacturer||'UNKNOWN')+pair('Spesifikasi',state?.asset?.specification||'UNKNOWN')+pair('Lokasi',state?.asset?.location||machine.area||'UNKNOWN')+pair('Status',truth.operatingStatus)+pair('3D source',truth.source3D)+pair('3D detail',truth.detail3D)+pair('Data confidence',truth.dataConfidence)+pair('Source count',TECHNICAL_SOURCES.length+' sumber')+pair('Posisi',truth.position)+pair('Dasar posisi',positionStatusLabel(policy.positionStatus))+pair('Sumber identitas',machine.source==='USER_CONFIRMED'?'Konfirmasi pengguna':'Registry mesin');
...
try{engine=new FactoryEngine($('#viewport'),part=>{const meta=taxonomyForPart(part);if(meta)selectedTaxonomyId=meta.id;choosePart(part);showPanel();renderPanel('structure');});engine.onTaxonomySelect=id=>selectTaxonomy(id,{revealPanel:false});engine.onSimulationUpdate=next=>{simulationState=next;updateSimulationPanel(next);};engine.onLODChange=updateLODStatus;updateLODStatus(engine.getLODState());simulationState=engine.getPrintingSimulationState();engine.onReset=()=>{explode=0;selectedPart=null;selectedTaxonomyId=ACTIVE_ROOT;engine.clearPreparedLODHint();renderPanel();};engine.onError=message=>toast(message,true);$('#engine-status').textContent='Menyiapkan denah pabrik';try{engine.setLow(localStorage.getItem('offset5-low')==='1'||matchMedia('(max-width:767px)').matches||matchMedia('(pointer:coarse) and (max-width:1024px)').matches);}catch{}}catch(e){renderStaticMachineFallback(e);$('#engine-status').textContent='Tampilan 3D belum tersedia';}
try{
