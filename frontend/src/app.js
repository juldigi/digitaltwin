...
import {validateSceneOverrides,validateSceneImport} from './scene-editor-state.js';
...
function settingsDialog(){
 ...
 on('#settings-connect',connectionDialog);
...
try{const config=await fetch('./config.json').then(r=>r.json());const savedBase=localStorage.getItem('offset5-api-base');apiBase=savedBase==='https://digitaltwin.offsetbmj.workers.dev'?config.apiBase:(savedBase||config.apiBase||'');if(savedBase==='https://digitaltwin.offsetbmj.workers.dev')localStorage.setItem('offset5-api-base',apiBase);if(cacheEnabled&&apiBase){const cached=await cache.get(apiBase);if(cached?.state){state=cached.state;cachedDataActive=true;engine?.loadLayout(activeLayout());renderStatus();renderPanel();updateConnectionTruth();toast('CACHED DATA · '+new Date(cached.savedAt).toLocaleString('id-ID'));}}else updateConnectionTruth();}catch(e){updateConnectionTruth();toast('Data tersimpan tidak dapat dibaca. Mode lokal tetap tersedia.',true);}
