...
function renderPanel(tab=activeTab){
 ...
 if(tab==='overview'){
  ...
  if(IS_OFFSET10){
   ...
   $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset','SBM-2')+pair('Pabrikan','LEXUS')+pair('Model plant','HSM-CTM7')+pair('Serial Number','00982')+pair('Functional Location','PC-PK2-OFS-SHT-SHEETING01')+pair('Tahun','2014')+pair('Kategori','Roll-to-sheet sheeter')}</dl><h3>Model 3D</h3><div class="card accent"><h4>Twin khusus Sheeting · orientasi proses terkoreksi</h4><p>Exterior V62 memprioritaskan visual resmi keluarga Lexus HSM 56 2014: satu rollstand/reel reference di sisi kanan, feed/tension/EPC dengan roller tersangga, enclosed cross-cut zone, narrow-belt transport, lalu portal lift-table stacker di sisi kiri.</p><span class="tag">DATABASE BMJ</span><span class="tag">RIGHT → LEFT</span><span class="tag">LEXUS HSM56 VISUAL REF</span></div><dl class="data-list">${pair('Alur material','RIGHT → LEFT')+pair('Input','Paper reel / fixed two-sided rollstand family reference · kanan')+pair('Pemotongan','Enclosed cross-cut zone · exact HSM-CTM7 unresolved')+pair('Output','Narrow-belt transport → portal lift table · kiri')+pair('Cut length family ref.','400–1700 mm')+pair('Knife load family ref.','600 gsm')+pair('Kecepatan family ref.','300 m/min')+pair('Referensi',TECHNICAL_SOURCES.length+' sumber')}</dl><div class="card"><h4>Batas ketelitian</h4><p>HSM-CTM7, serial, SAP code dan tahun adalah identitas database BMJ. Exterior V62 memprioritaskan brochure/foto HSM 56 2014 sebagai visual family reference. Lexus Indonesia dipakai untuk fungsi tension/EPC, sementara exact rollstand dan knife HSM-CTM7 tetap unresolved dan tidak diklaim sebagai OEM-verified.</p></div>`;
  }else{
...
function machineDetailDialog(machine){
 ...
 if(machine.has3D)on('#open-machine-3d',()=>{const route=machine.machineId==='BMJ-MCH-0009'?'offset10':machine.machineId==='BMJ-MCH-0010'?'apm2':machine.machineId==='BMJ-MCH-0002'?'sheeting':machine.machineId==='BMJ-MCH-0003'?'offset5':machine.machineId;if(route!==MACHINE_KEY){location.href=`./?machine=${encodeURIComponent(route)}&v=62`;return;}closeModal();setView('machine');showPanel();renderPanel('overview');});
}
