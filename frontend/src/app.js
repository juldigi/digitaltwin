...
let REQUESTED_MACHINE=null,GENERIC_CONFIG=null,MACHINE_KEY=null,IS_OFFSET10=false,IS_APM2=false,IS_SHEETING=false,IS_GENERIC=false,IS_VERIFIED_REGISTRY_SIM=false,GENERIC_TAXONOMY=[],GENERIC_ROOT=null,ACTIVE_ROOT=null,ACTIVE_TAXONOMY=[],TAXONOMY_BY_ID=new Map(),taxonomyChildren=()=>[],taxonomyStats=emptyTaxonomyStats,PHOTO_REGISTRY=[],GENERIC_SOURCES=[],TECHNICAL_SOURCES=[],photoStats=emptyPhotoStats,ORIENTATION=neutralOrientation,PRINTING_SIMULATION_STAGES=[],INK_SIMULATION_SEQUENCE=[];
function clearActiveMachineDescriptor(){
 REQUESTED_MACHINE=null;GENERIC_CONFIG=null;MACHINE_KEY=null;IS_OFFSET10=false;IS_APM2=false;IS_SHEETING=false;IS_GENERIC=false;IS_VERIFIED_REGISTRY_SIM=false;
...
function applyActiveMachineState(){
 ...
 simulationState={available:Boolean(MACHINE_KEY),active:false,running:false,paused:false,speed:1,stage:MACHINE_KEY?'Feeder':null,completed:0,progress:0,sheetsVisible:0,pileSheetsVisible:0,rotorCount:0,oscillatorCount:0,mechanismCount:0,inkFlowCount:0,uvLampCount:0,uvActive:false,pathVisible:IS_SHEETING?false:Boolean(MACHINE_KEY),inkFlowVisible:Boolean(MACHINE_KEY)};
 if(!MACHINE_KEY){
  state.asset={...state.asset,asset_id:null,asset_code:null,codename:null,model:null,description:'Belum memilih mesin',source_description:null,manufacturer:null,specification:null,configuration:null,'3d_status':'BELUM MEMILIH MESIN',data_confidence:null,discovery_status:null,sources:[]};
  return;
 }
...
function showPanel(){window.BMJAppState?.setInspector?.(true,activeTab);emitDomainState({inspectorState:{open:true,tab:activeTab}});}
...
function showHome({historyMode='none'}={}){
 ...
 window.BMJAppState?.setInspector?.(false,'overview');
 $$('.rail>button').forEach(b=>b.classList.remove('active'));
...
function openSceneEditor(){
 ...
 const onEditorKeyUp=e=>{if(!keyboardMoveActive||!editorArrowKeys.includes(e.key))return;keyboardMoveActive=false;update();};
 ...
 panel.querySelector('#se-edit-machine-parts')?.addEventListener('click',async()=>{const machine=MACHINE_REGISTRY_BY_ID.get(editorMachineId);if(!machine||!canOpenTechnical3D(machine))return toast('Model bagian mesin ini belum tersedia.',true);try{engine.gizmo.detach();isolationGuard.restore();selected=null;isolated=false;previewOriginal=false;await engine.switchMachine(machineRoute(machine));engine.setView('machine',state);engine.setSceneEditing(true);engine.applySceneOverrides(overrides);editorScope='machine';editorCategory='components';update();}catch(error){toast('Bagian mesin belum dapat dibuka: '+error.message,true);}});
 panel.querySelector('#se-back-machine')?.addEventListener('click',()=>{engine.gizmo.detach();isolationGuard.restore();selected=null;isolated=false;previewOriginal=false;editorScope='factory';editorCategory='machines';engine.setView('factory',state);engine.applySceneOverrides(overrides);engine.setSceneEditing(true);const id=editorMachineId&&engine.actualFactory?.assets?.has(editorMachineId)?'asset:'+editorMachineId:null;if(id)engine.selectSceneObject(id);else update();});
 ...
 panel.querySelector('#se-close').onclick=()=>{if(JSON.stringify(overrides)!==JSON.stringify(state.sceneOverrides||{})&&!confirm('Perubahan yang belum disimpan akan dibuang. Tutup Editor?'))return;isolated=false;isolationGuard.restore();Object.keys(overrides).forEach(k=>delete overrides[k]);Object.assign(overrides,state.sceneOverrides||{});engine.applySceneOverrides(state.sceneOverrides||{});engine.gizmo.setTranslationSnap(null);engine.gizmo.setRotationSnap(null);engine.setSceneEditing(false);engine.onSceneSelect=null;engine.onSceneTransform=null;engine.gizmo.removeEventListener('mouseDown',onDragStart);engine.gizmo.removeEventListener('mouseUp',onDragEnd);document.removeEventListener('keydown',onEditorKeyDown);document.removeEventListener('keyup',onEditorKeyUp);engine.setView('factory',state);panel.remove();};
...
window.addEventListener('resize',()=>redrawPlantPlan(),{passive:true});window.addEventListener('bmj:statechange',event=>{if(document.body.classList.contains('workspace-2d'))redrawPlantPlan(event.detail?.selectedAsset||null);});if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});
