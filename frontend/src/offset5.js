...
export class OffsetMachineTemplate {
  ...
  markExteriorCover(object){if(object)object.userData.exteriorCover=true;return object;}
  markServiceDetail(object){if(object)object.userData.serviceDetail=true;return object;}
  ...
  grille(g,pos,width,height,axis='x'){
    ...
    if(axis==='x'){
      cover(this.box(g,[.035,height,width],[x,y,z],'black'));
      for(let i=0;i<13;i++)cover(this.box(g,[.05,.014,width],[x+.025,y-height/2+i*height/12,z],'steel'));
      for(let i=0;i<9;i++)cover(this.box(g,[.05,height,.012],[x+.028,y,z-width/2+i*width/8],'graphite'));
    }else{
      cover(this.box(g,[width,height,.035],[x,y,z],'black'));
      for(let i=0;i<10;i++)cover(this.box(g,[width,.014,.05],[x,y-height/2+i*height/9,z+.025],'steel'));
    }
  }
  ...
  batchMeshes(){
    // Never merge removable housing with structural/support geometry. The cutaway mode
    ...
    for(const group of this.nodes){
      ...
      for(const {material,cover,semantic,visible,meshes} of batches.values()){
        ...
        if(semantic!=='normal'){mesh.userData[semantic]=true;mesh.userData.uvElementCount=meshes.reduce((sum,m)=>sum+(m.userData.uvElementCount||1),0);}
  ...
  build(){
    ...
    this.markExteriorCover(this.box(board,[1.0,1.00,1.94],[0,.77,0],'graphite',.035));
    this.markExteriorCover(this.box(board,[1.0,.055,1.88],[0,1.30,0],'steel'));
  ...
  printingUnitInternals(g,i){
    ...
    // Sheet travel is rendered only by the live Printing Test simulation. No static paper plane is kept inside the PU.
    ...
    const access=this.markExteriorCover(this.group(g,`${id}-service-access`,`${label} · service access & guards`,[0,0,0],[.20,.15,.70],['IMG_1627.jpeg','IMG_1628.jpeg','IMG_2389(1).jpeg'],'Guard boundaries follow the photographed operator/drive-side access. Interlocks and lubrication points remain reference-only.'));
  ...
  feeder(x){
    ...
    this.markExteriorCover(this.box(feed,[.4,.37,.62],[-.15,1.98,0],'light',.025));
  ...
  coatingUnit(x){
    ...
    const op=this.markExteriorCover(this.group(g,'coater-operator-cover','Coater operator-side silver cover',[0,0,0],[.15,.20,1.0],photos));
    ...
    const service=this.markExteriorCover(this.group(g,'coater-service','Coater drive-side service panel',[0,0,0],[.10,.15,-1.1],['IMG_2389(1).jpeg','IMG_2390(1).jpeg']));
  ...
  downstreamAccess(D){
    ...
    // V48: no standalone operator step between PU8 and coater. The global gallery already
    // provides access; this transition is supported by a low central machine plinth only.
    this.box(puCoater,[puCoaterGap,.12,1.62],[0,.06,0],'graphite',.018);
  ...
  dryerExtension(x){
    ...
    g.scale.x=OFFSET5_DIMENSIONS.layout.dryerLength/1.62;
    this.markExteriorCover(this.box(g,[1.55,.92,2.02],[0,.72,0],'graphite',.035));
    const uvSupport=this.group(g,'dryer-uv-support-frame','UV module structural support frame',[0,0,0],[0,.20,0],photos,'Low-profile rails and crossmembers support the UV cassette zone; this is machine structure, not an operator footstep.');
    for(const z of [-.66,.66])this.box(uvSupport,[1.46,.07,.10],[0,1.205,z],'graphite',.010);
    for(const x0 of [-.50,0,.50])this.box(uvSupport,[.10,.07,1.42],[x0,1.205,0],'steel',.008);
    const hood=this.markExteriorCover(this.group(g,'dryer-hood','Sloped dryer / extension hood',[0,0,0],[.35,.55,0],photos));
    ...
    const lamps=this.group(g,'dryer-modules','Dryer / UV curing modules',[0,0,0],[0,.58,-.25],photos,'Installed UV drying is user-confirmed. The cassette arrangement is a functional visual reconstruction; exact lamp count, wavelength, power and cassette model remain installation-specific.');
    const uv=this.group(lamps,'dryer-uv-system','UV curing system · lamp cassettes, reflector & cooling gap',[0,0,0],[0,.34,-.18],photos,'Heidelberg DryStar UV uses short-distance UV cassettes, reflectors and temperature management. Geometry is fitted inside the photographed dryer hood; electrical ratings are not asserted.');
    for(const x0 of [-.42,0,.42]){
      this.box(uv,[.30,.11,1.44],[x0,1.50,0],'graphite',.014);
      const reflector=this.box(uv,[.24,.035,1.34],[x0,1.435,0],'steel',.008);reflector.userData.uvReflector=true;
      const lamp=this.box(uv,[.13,.025,1.26],[x0,1.405,0],'light',.006);lamp.userData.uvLamp=true;lamp.material.emissive.setHex(0x5d36ff);lamp.material.emissiveIntensity=.08;
      const window=this.box(uv,[.22,.018,1.30],[x0,1.382,0],'glass',.004);window.userData.uvWindow=true;
      const beam=this.box(uv,[.20,.17,1.26],[x0,1.29,0],'blue',.004);beam.userData.uvBeam=true;beam.material.color.setHex(0x6f4bff);beam.material.emissive.setHex(0x5a2cff);beam.material.emissiveIntensity=.7;beam.material.opacity=.18;beam.material.transparent=true;beam.material.depthWrite=false;beam.visible=false;
    }
  ...
  delivery(x){
    ...
    g.scale.x=OFFSET5_DIMENSIONS.layout.deliveryBodyLength/1.995;
    ...
    this.markExteriorCover(this.box(frame,[1.84,.34,2.20],[.04,2.23,0],'light',.035));
    this.markExteriorCover(this.box(frame,[1.55,.46,.10],[.00,2.07,-1.10],'light',.018));
    this.markExteriorCover(this.box(frame,[1.05,.34,.025],[.10,2.07,-1.165],'glass',.008));
    ...
    const paperStack=this.group(pile,'delivery-paper-stack','Delivery paper stack / receiving surface',[0,0,0],[0,.10,0],['IMG_2312.jpeg','IMG_1656.jpeg'],'Dedicated receiving stack target for the running simulation; static pile proportions remain photo-derived.');
    this.box(paperStack,[1.20,.96,1.64],[.08,.73,0],'paper',.010);
    for(let n=0;n<24;n++)this.box(paperStack,[1.205,.006,1.645],[.08,.29+n*.038,0],'light');
  ...
  showOnly(parts=[],on=true){
    const list=(parts||[]).filter(Boolean);
    for(const n of this.nodes)n.visible=!on||!list.length||list.some(part=>n===part||this.contains(n,part));
  }
