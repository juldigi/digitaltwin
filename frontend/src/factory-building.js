...
emptyPalletStack(93.2,84.8,4,'RMS');mobilePaperTrolley(92.9,76.8,'RMS');floorScale(93.0,72.8,'RMS');
 ...
 Object.assign(buildingDetailStats,{v205IpalPhotoObjects:0,v205IpalLegacyHidden:0,v205IpalPipingRuns:0,v205IpalSafetyDetails:0,v205IpalVegetationObjects:0,v205IpalWeatheringDetails:0,v206SyntheticCrossUnitPipesHidden:0,v206OpaqueBasins:0,v206HerringboneSegments:0,v206OperatorFacadeCorrections:0,v206RemovedUnsupportedTankLadders:0,v206ChemicalTankCount:0});
 ...
 for(const o of [...ipal.children]){
  ...
  o.visible=false;
  o.userData={...o.userData,supersededByV206:true,supersededReason:'ACTUAL_IPAL_PHOTOS_2511_2525_OVERRIDE_PRE_V206_REFERENCE',legacySemantic:sem};
  buildingDetailStats.v205IpalLegacyHidden++;
 }
 for(const o of layers.roof.children){
  const sem=String(o.userData?.semantic||'');
  if(legacyIpalPattern.test(sem)){o.visible=false;o.userData={...o.userData,supersededByV206:true,supersededReason:'ACTUAL_IPAL_PHOTOS_2511_2525_OVERRIDE_PRE_V206_ROOF_REFERENCE'};buildingDetailStats.v205IpalLegacyHidden++;}
 }
 ...
 const paverSegments=[];
 for(let x=33.75,row=0;x<59.05;x+=.62,row++){
  for(let z=-117.45,col=0;z<-104.35;z+=.36,col++){
   const flip=(row+col)%2===0,dx=.25,dz=.16;
   paverSegments.push(x-dx,.117,z-(flip?dz:-dz),x+dx,.117,z+(flip?dz:-dz));
  }
 }
 const paverGeo=new T.BufferGeometry();paverGeo.setAttribute('position',new T.Float32BufferAttribute(paverSegments,3));
 const paverJoints=new T.LineSegments(paverGeo,new T.LineBasicMaterial({color:0x707875,transparent:true,opacity:.34}));ipalPhoto.add(paverJoints);photoTag(paverJoints,'IPAL_PHOTO_HERRINGBONE_PAVER_JOINTS',{batched:true,photoRule:'HERRINGBONE_INTERLOCKING_REFERENCE'});buildingDetailStats.v206HerringboneSegments=paverSegments.length/6;
 ...
 const makeBlueBasin=(spec,semantic,waterColor)=>{
  ...
  const shadow=gb(0,h-.24,0,w-.36,.018,d-.36,0x24383d,'INTERIOR_DEPTH_SHADOW',.34,{photoRule:'NO_VISIBLE_OPEN_WATER_SURFACE_IN_SOURCE_PHOTOS',notWaterSurface:true});
 ...
 buildingDetailStats.v206RemovedUnsupportedTankLadders++; // No vertical ladder is asserted: the photos do not show a confirmed ladder on this tank face.
