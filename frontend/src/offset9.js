...
export class Offset9MachineTemplate{
 ...
 buildFeeder(){
  ...
  const venturi=this.group(reg,'offset9-register-venturi','Venturi infeed nozzles');venturi.userData.oemFunction='VENTURI_SHEET_INFEED';for(const z of [-.42,-.14,.14,.42]){const n=this.cyl(venturi,.012,.07,[.46,1.245,z],'cyan','venturi-nozzle','y');n.userData.airNozzle=true;n.userData.feederAirNozzle=true;}
 ...
 printUnit(parent,module,index){
  ...
  const autoplate=this.group(g,id+'-autoplate','AutoPlate semi-automatic interface');autoplate.userData.familyFeature=true;autoplate.userData.installedSystemVerified=false;this.box(autoplate,[.20,.09,1.80],[-.31,1.90,0],'blue');this.box(autoplate,[.055,.055,1.58],[-.20,1.77,0],'steel',.005);for(const z of [-.68,.68])this.box(autoplate,[.10,.14,.10],[-.20,1.82,z],'graphite',.008);
  ...
  const jacket=this.group(transfer,id+'-transfer-jacket','TransferJacket Blue family surface reference');jacket.userData.familyFeature=true;jacket.userData.installedVariantVerified=false;const jm=this.cyl(jacket,.219,1.69,[.33,.52,0],'blue','transfer-jacket-reference');jm.userData.familySurfaceReference=true;
  ...
  const fountain=this.group(g,id+'-ink-fountain','Remote-controlled Heidelberg ink fountain and liner');fountain.userData.oemFunction='REMOTE_CONTROLLED_INK_FOUNTAIN_WITH_LINER';fountain.userData.exactZoneCountVerified=false;this.box(fountain,[.60,.18,1.47],[-.25,2.34,0],'graphite');this.box(fountain,[.52,.018,1.39],[-.22,2.27,0],'silver',.003);this.box(fountain,[.08,.08,1.35],[-.48,2.22,0],'blue',.006);for(const [j,x,y] of [[0,-.37,2.08],[1,-.20,2.13],[2,-.02,2.15],[3,.16,2.12],[4,.33,2.05]])this.cyl(ink,.047,1.41,[x,y,0],j%2?'rubber':'steel','ink-transfer');for(const [j,x,y] of [[0,-.22,1.94],[1,-.07,1.99],[2,.09,1.98],[3,.23,1.91]])this.cyl(ink,.052+j*.002,1.42,[x,y,0],'rubber','ink-form-'+(j+1));
