...
export class Offset9MachineTemplate{
 ...
 printUnit(parent,module,index){
  ...
  const plate=this.group(train,id+'-plate','Plate cylinder');this.cyl(plate,.178,1.72,[-.12,1.68,0],'steel','plate');
  const autoplate=this.group(g,id+'-autoplate','AutoPlate semi-automatic interface');autoplate.userData.familyFeature=true;autoplate.userData.installedSystemVerified=false;const apRail=this.box(autoplate,[.20,.09,1.80],[-.31,1.90,0],'blue');apRail.userData.mechanismRole='autoplate-guide-reference';const apClamp=this.box(autoplate,[.055,.055,1.58],[-.20,1.77,0],'steel',.005);apClamp.userData.mechanismRole='autoplate-clamp-reference';for(const z of [-.68,.68])this.box(autoplate,[.10,.14,.10],[-.20,1.82,z],'graphite',.008);
  ...
  const jacket=this.group(transfer,id+'-transfer-jacket','TransferJacket Blue family surface reference');jacket.userData.familyFeature=true;jacket.userData.installedVariantVerified=false;const jm=this.cyl(jacket,.219,1.69,[.33,.52,0],'blue','transfer-jacket-reference');jm.userData.familySurfaceReference=true;jm.userData.mechanismRole='transferjacket-blue-family-reference';
  const ink=this.group(g,id+'-inking','Speed-compensated inking unit · visualization reference');ink.userData.exactRollerCountVerified=false;
  const fountain=this.group(g,id+'-ink-fountain','Remote-controlled Heidelberg ink fountain and liner');fountain.userData.oemFunction='REMOTE_CONTROLLED_INK_FOUNTAIN_WITH_LINER';fountain.userData.exactZoneCountVerified=false;const fb=this.box(fountain,[.60,.18,1.47],[-.25,2.34,0],'graphite');fb.userData.mechanismRole='heidelberg-ink-fountain-body';const liner=this.box(fountain,[.52,.018,1.39],[-.22,2.27,0],'silver',.003);liner.userData.mechanismRole='ink-fountain-liner-reference';const keyRail=this.box(fountain,[.08,.08,1.35],[-.48,2.22,0],'blue',.006);keyRail.userData.mechanismRole='remote-ink-zone-interface-reference';for(const [j,x,y] of [[0,-.37,2.08],[1,-.20,2.13],[2,-.02,2.15],[3,.16,2.12],[4,.33,2.05]])this.cyl(ink,.047,1.41,[x,y,0],j%2?'rubber':'steel','ink-transfer');for(const [j,x,y] of [[0,-.22,1.94],[1,-.07,1.99],[2,.09,1.98],[3,.23,1.91]])this.cyl(ink,.052+j*.002,1.42,[x,y,0],'rubber','ink-form-'+(j+1));
  const damp=this.group(g,id+'-dampening','Alcolor film dampening · visualization reference');damp.userData.exactRollerCountVerified=false;damp.userData.varioFamilyFeature=true;for(const [j,x,y] of [[0,.26,1.74],[1,.39,1.83],[2,.45,1.69],[3,.39,1.54],[4,.27,1.43]])this.cyl(damp,.044+(j>2?.007:0),1.40,[x,y,0],j%2?'rubber':'steel','damp-'+(j+1));this.box(damp,[.34,.07,1.46],[.34,1.34,0],'steel');
  g.userData.automaticWashupFamilyFeature=true;g.userData.automaticWashupInstalledVerified=false;
 }
 ...
 buildDelivery(){
  ...
  const stack=this.group(g,'offset9-delivery-stack','Delivery pile lift');this.box(stack,[1.18,.07,1.24],[.67,.46,0],'steel');const paper=this.group(stack,'offset9-delivery-paper-stack','Printed sheet receiving stack');this.box(paper,[1.14,.48,1.18],[.67,.74,0],'paper');
 }
 refineExteriorV232(){
