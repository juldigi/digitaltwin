...
for(const [id,name,ref] of [['FEED','Feeder and sheet register','offset9-feeder'],['PRINT','Four offset printing units','offset9-print'],['COAT','Inline coating','offset9-coat'],['DELIVERY','Delivery · standard/high-pile option unverified','offset9-delivery'],['ACCESS','Frames, guards and access','offset9-access']])add(`O9.${id}`,name,2,'O9',[ref]);
for(const m of OFFSET9_MODULE_SEQUENCE){
 ...
 const systems=m.type==='print'?
  [['INK','Ink fountain and speed-compensated inking · represented roller count unverified','inking'],['DAMP','Alcolor film dampening · represented roller count unverified','dampening'],['PLATE','Plate cylinder and AutoPlate interface','plate'],['BLANKET','Blanket cylinder and wash device','blanket'],['IMPRESSION','Impression cylinder and jacket','impression'],['TRANSFER','Sheet grippers and transfer','transfer']]:
...
for(const [branch,id,name,ref,items] of [
 ...
 ['DELIVERY','CHAIN','Gripper-chain transport','offset9-delivery-chain',['Chain rail','Sprocket','Drive reference']],
 ['DELIVERY','GRIPPER','Delivery gripper bars','offset9-delivery-grippers',['Gripper bar','Gripper fingers','Chain attachment']],
