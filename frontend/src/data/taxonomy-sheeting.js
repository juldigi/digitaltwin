...
const nodes=[];
add(nodes,'SH',null,1,'Mesin','SHEETING LEXUS · HSM-CTM7',['MACHINE-SHEETING'],'Identitas BMJ verified. V67 memakai anchor visual komponen dari brochure/foto resmi HSM 56 2014 dan mempertahankan RIGHT→LEFT dari konfirmasi pengguna. Exact internal HSM-CTM7 tidak dinaikkan menjadi verified tanpa foto/drawing spesifik.','VERIFIED');
const chains=[
...
add(nodes,'SH.HEAD.SUB.BLOCK.PART.WINDOW','SH.HEAD.SUB.BLOCK.PART',6,'Spesifik Part','Panoramic Window Assembly',['sheeting-window'],'V67 makes this a selectable true aperture: glass, frame and handles have no opaque side slab directly behind them.');
...
add(nodes,'SH.DEL.SUB.BLOCK.PART.FAST','SH.DEL.SUB.BLOCK.PART',6,'Spesifik Part','Fast Tape Separation Zone',['sheeting-fast-belts'],'V67 process-zone geometry; exact Lexus drive ratios are not claimed.');
add(nodes,'SH.DEL.SUB.BLOCK.PART.SLOW','SH.DEL.SUB.BLOCK.PART',6,'Spesifik Part','Slow Tape Transfer Zone',['sheeting-slow-belts'],'V67 process-zone geometry based on generic folio-sheeter transport architecture.');
add(nodes,'SH.DEL.SUB.BLOCK.PART.OVERLAP','SH.DEL.SUB.BLOCK.PART',6,'Spesifik Part','Overlap Tape Zone',['sheeting-overlap-belts'],'V67 process-zone geometry used to make downstream sheet overlap coherent.');
...
add(nodes,'SH.STACK.SUB.BLOCK.PART.JOG','SH.STACK.SUB.BLOCK.PART',6,'Spesifik Part','Stack Alignment / Jogger Reference',['sheeting-stacker-joggers'],'V67 process-reference geometry adds front/back stop and side jogger motion so sheets square into the pile. These are generic folio-sheeter functions, not claimed as exact HSM-CTM7 hardware.');
