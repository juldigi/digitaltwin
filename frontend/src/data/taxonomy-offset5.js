...
for(let unit=1;unit<=8;unit++){
 ...
 for(const [key,name,ref] of [['OP','Operator Access Steps',`press-${i}-steps`],['DRIVE','Drive-side Access Steps',`press-${i}-drive-steps`]]){
  ...
  if(i<7){
   ...
   add(`${pid}.S3`,pid,6,'Spesifik Part',key==='OP'?'Middle step & full inter-unit landing':'Mirrored middle step to shared landing',{meshRefs:[ref],sourceRefs:photo,confidence:CONFIDENCE.PHOTO_VERIFIED,explodeVector:[.04,.03,.08],maintenanceTag:'VISUAL_INSPECTION'});
   add(`${pid}.S4`,pid,6,'Spesifik Part',key==='OP'?'Supported inter-unit landing edge':'Shared gripper-cover landing interface',{meshRefs:[ref],sourceRefs:photo,confidence:CONFIDENCE.PHOTO_VERIFIED,explodeVector:[.04,.10,-.08],maintenanceTag:'VISUAL_INSPECTION'});
  }
 }
}
...
for(let n=1;n<=7;n++){
 ...
 const blocks=[
  ['DRUM','Transfer Drum',[base],[['DRUM','Transfer Drum',base,'Transfer drum body / sheet handover reference']]],
  ['GRIPPER','Gripper System',[`${base}-gripper-a`,`${base}-gripper-b`,`${base}-gripper-shaft`,`${base}-gripper-spring`,`${base}-gripper-cam`],[
   ['BAR_A','Gripper Bar A',`${base}-gripper-a`,'Gripper fingers / contact edge reference'],
   ['BAR_B','Gripper Bar B',`${base}-gripper-b`,'Gripper fingers / contact edge reference'],
   ['SHAFT','Gripper Shaft & Supports',`${base}-gripper-shaft`,'Shaft support / bearing interface reference'],
   ['SPRING','Return Spring & Pivot',`${base}-gripper-spring`,'Return-spring preload / pivot reference'],
   ['CAM','Opening Cam / Follower / Lever',`${base}-gripper-cam`,'Cam profile / follower / lever timing reference']
  ]],
  ['GUIDE','Sheet Guide',[`${base}-guide`],[['GUIDE','Sheet Guide',`${base}-guide`,'Sheet guide surface / clearance reference']]]
 ];
 for(const [key,name,refs,components] of blocks){
  ...
  components.forEach(([componentKey,componentName,ref,specificName],i)=>{
   const pid=`${bid}.P${i+1}`;add(pid,bid,5,'Part',componentName,{meshRefs:[ref],sourceRefs:manual,confidence:CONFIDENCE.MEDIUM,explodeVector:[.08,.08,(i-2)*.10],description:'Mapped to a distinct modeled transfer component; exact service dimensions and timing remain reference-only.'});
   add(`${pid}.S1`,pid,6,'Spesifik Part',specificName,{meshRefs:[ref],sourceRefs:manual,confidence:CONFIDENCE.REFERENCE_ONLY,explodeVector:[.05,.05,.08],description:'Anchored to the modeled component envelope for inspection navigation; exact adjustment values are not asserted.',maintenanceTag:'VISUAL_INSPECTION'});
  });
 }
}
...
for(const [key,name,refs,parts] of [
 ...
 ['ACCESS','PU8 / Coater Operator Access',['pu8-coater-access'],['Enclosed Sheet Transfer','Supported Checker-plate Landing','Lower Access Step']],
...
for(const [key,name,refs,parts] of [
 ...
 ['MODULE','UV / Dryer Modules',['dryer-modules','dryer-uv-system','dryer-ventilation','dryer-air-plenum','dryer-monitoring'],['UV Lamp Cassettes','Reflector / Quartz Window','Cooling / Extraction Plenum','Temperature / Airflow Monitoring Points']],
 ['SERVICE','Coater / Dryer Service Transition',['coater-dryer-service-bay'],['Enclosed Sheet-path Transition','Supported Side Deck','Protective Housing']],
...
for(const [key,name,refs,parts] of [
 ...
 ['ACCESS','Dryer / Inspection / Delivery Access',['dryer-delivery-access'],['Enclosed Receiving Transition','Supported Service Landing','Approach Step']]
]){
