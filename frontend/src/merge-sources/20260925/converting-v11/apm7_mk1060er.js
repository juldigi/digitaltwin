...
const STATIONS=[
  ...
  },
  {
    ...
    "height": 1.56,
    "components": [
      "upper stripping board",
      "middle frame",
      "lower pins",
      "cam-driven stripping motion",
      "waste openings"
    ]
  },
  {
    "id": "blanking",
    "label": "Blank separation",
    "x": 4.58,
    "height": 1.62,
    "components": [
      "upper separation tool",
      "lower blank delivery device",
      "support grid",
      "product separation tooling"
    ]
  },
  {
    "id": "waste",
    "label": "Waste conveyor",
    "x": 5.55,
    "height": 0.72,
    "components": [
      "scrap drop chute",
      "independent waste belt",
      "side discharge"
    ]
  },
  {
...
function build(T,options={}){if(!T)throw new Error(MACHINE_ID+' requires THREE');const root=new T.Group();root.name=MACHINE_ID;root.userData={machineId:MACHINE_ID,name:"AUTOPLATEN 7 STRIPPING & BLANKING",model:"MK 1060 ER",spec:SPEC,taxonomy:taxonomy(),motions:MOTIONS,visualDNA:VISUAL_DNA,evidenceStatus:"REFERENCE_VERIFIED_MODEL_FAMILY"};const mm=materials(T);addBase(T,root,mm);addStations(T,root,mm);addServices(T,root,mm);return root;}
