...
export class FZ1200ProcessSimulation{
 ...
 state(){
  ...
  return {available:true,blocked:false,active:this.active,running:this.running,paused:this.paused,speed:this.speed,stage:FZ1200_SIMULATION_STAGES[index],completed:this.completed,progress,
   ...
   clamped:this.clamped,liftActive:this.liftActive,turningActive:this.turningActive,turnAngleDeg:this.turnAngleDeg,airingActive:this.airingActive,joggingActive:this.joggingActive,hydraulicActive:this.hydraulicActive,
 ...
 resetMechanisms(){
  ...
  this.turnAngleDeg=0;this.clamped=this.liftActive=this.turningActive=this.airingActive=this.joggingActive=this.hydraulicActive=false;
 }
 ...
 update(now){
  ...
  const clampProfile=smooth(phase,.10,.19)*(1-smooth(phase,.90,.98)),lift=smooth(phase,.20,.29)*(1-smooth(phase,.80,.91)),turn=smooth(phase,.30,.49);
  const air=phase>.50&&phase<.69,jog=phase>.59&&phase<.78,hyd=clampProfile>.02||lift>.02;
  ...
  this.completed=cycleIndex;this.clamped=clampProfile>.94;this.liftActive=lift>.05;this.turningActive=phase>.30&&phase<.50;this.turnAngleDeg=turn*180;this.airingActive=air;this.joggingActive=jog;this.hydraulicActive=hyd;
