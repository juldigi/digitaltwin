...
export class SharkN650ProcessSimulation{
 ...
 resetFlags(){this.feederSuctionActive=false;this.feederSuctionReferenceOnly=true;this.transferVacuumReady=false;this.transportEncoderActive=false;this.blankPresenceTrigger=false;this.lightingProgramReady=false;this.cameraTriggerActive=false;this.captureComplete=false;this.scanActive=false;this.processingActive=false;this.processingComplete=false;this.decisionReady=false;this.rejectPermit=false;this.rejectConfirmed=false;this.goodReturnConfirmed=false;this.badReturnConfirmed=false;this.negativePitchCapabilityReference=true;this.negativePitchActive=false;this.interlockSafe=true;this.rejectTrackingActive=false;this.demoRejectActive=false;this.goodRoutingActive=false;this.badRoutingActive=false;this.demoRejectOnly=true;this.demoRejectActuator='PLATE_REFERENCE';this.activeRejectTrackingIds=[];this.activePassTrackingIds=[];}
 state(){
  ...
  return {available:true,blocked:false,active:this.active,running:this.running,paused:this.paused,speed:this.speed,stage:SHARK_N650_SIMULATION_STAGES[index],completed:this.completed,rejectedDemo:this.rejected,inspectedDemoCount:this.inspectedDemoCount,progress:p,
   ...
   feederSuctionActive:this.feederSuctionActive,feederSuctionReferenceOnly:this.feederSuctionReferenceOnly,transferVacuumReady:this.transferVacuumReady,transportEncoderActive:this.transportEncoderActive,blankPresenceTrigger:this.blankPresenceTrigger,lightingProgramReady:this.lightingProgramReady,cameraTriggerActive:this.cameraTriggerActive,captureComplete:this.captureComplete,scanActive:this.scanActive,processingActive:this.processingActive,processingComplete:this.processingComplete,decisionReady:this.decisionReady,rejectPermit:this.rejectPermit,rejectConfirmed:this.rejectConfirmed,goodReturnConfirmed:this.goodReturnConfirmed,badReturnConfirmed:this.badReturnConfirmed,negativePitchCapabilityReference:this.negativePitchCapabilityReference,negativePitchActive:this.negativePitchActive,interlockSafe:this.interlockSafe,rejectTrackingActive:this.rejectTrackingActive,demoRejectActive:this.demoRejectActive,goodRoutingActive:this.goodRoutingActive,badRoutingActive:this.badRoutingActive,
 ...
 updateBlanks(){
  let scan=0,processing=0,processed=0,decisions=0,rejectTracked=false,rejectAtGate=false,rejectConfirmed=false,goodRoute=false,badRoute=false,goodConfirmed=false,badConfirmed=false,feed=false,trigger=false;const rejectIds=[],passIds=[];const base=this.elapsed/7.2;
  for(const b of this.blanks){
   const raw=base+b.phase,lap=Math.floor(raw),t=((raw%1)+1)%1;if(lap>b.lap){if(b.lap>=0)this.outputPreviousResult(b);b.lap=lap;b.result=null;b.captured=false;b.processed=false;b.decisionReady=false;b.inspectedLap=-1;}
   if(t<.16)feed=true;if(t>=.24&&t<.31)trigger=true;
   if(t>=.31&&t<.49){scan++;b.captured=true;}
   if(t>=.49&&t<.63&&b.captured){processing++;if(t>=.59)b.processed=true;}
   if(b.processed&&!b.decisionReady&&t>=.63){this.assignInspectionResult(b,lap);b.decisionReady=true;}
   ...
   if(t>=.70&&b.decisionReady&&b.result){const q=clamp((t-.70)/.18);if(b.result==='REJECT_DEMO'){b.mesh.position.z=THREE.MathUtils.lerp(0,.62,q);rejectTracked=true;badRoute=true;if(b.trackingId)rejectIds.push(b.trackingId);if(t>=.74&&t<.88)rejectAtGate=true;if(t>=.86)rejectConfirmed=badConfirmed=true;}else{b.mesh.position.z=THREE.MathUtils.lerp(0,-.18,q);goodRoute=true;if(b.trackingId)passIds.push(b.trackingId);if(t>=.90)goodConfirmed=true;}}
   b.mesh.material.emissive?.setHex(t>=.31&&t<.49?0x2f7a9d:b.result==='REJECT_DEMO'&&t>=.63?0x6e2d27:0);b.mesh.material.emissiveIntensity=t>=.31&&t<.49?.78:b.result==='REJECT_DEMO'&&t>=.63?.42:0;
  }
  this.feederSuctionActive=feed;this.transferVacuumReady=this.active;this.transportEncoderActive=this.active;this.blankPresenceTrigger=trigger;this.lightingProgramReady=scan>0;this.cameraTriggerActive=trigger;this.captureComplete=this.blanks.some(b=>b.captured);this.scanActive=scan>0;this.processingActive=processing>0;this.processingComplete=processed>0;this.decisionReady=decisions>0;this.rejectPermit=rejectAtGate&&decisions>0;this.rejectConfirmed=rejectConfirmed;this.goodReturnConfirmed=goodConfirmed;this.badReturnConfirmed=badConfirmed;this.negativePitchActive=false;this.rejectTrackingActive=rejectTracked;this.demoRejectActive=this.rejectPermit;this.goodRoutingActive=goodRoute;this.badRoutingActive=badRoute;this.activeRejectTrackingIds=rejectIds;this.activePassTrackingIds=passIds;this.interlockSafe=(!this.demoRejectActive||this.rejectPermit)&&(!this.processingActive||this.captureComplete);
  this.updateOptics(this.scanActive);this.updateSuckers(feed);
