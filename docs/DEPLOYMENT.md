# Deployment GitHub + Cloudflare — Digital Twin V59

## 1. Source dan frontend build

Repositori aktif: `juldigi/digitaltwin` (**private**).

1. Push ke `main` menjalankan workflow **Verify frontend V59** untuk `npm ci`, build, dan seluruh regression test.
...
- URL Worker memuat **Factory Digital Twin V59**.
- Daftar Mesin menampilkan **41 equipment**.
- Offset 5, Offset 10, APM 2, dan Sheeting Lexus membuka geometry khusus masing-masing.
- Sheeting V59: dua posisi reel pada shaftless-unwind reference, open feed/tension/EPC bridge, compact cutter head, long tape/overlap delivery, dan layboy lift table. Continuous web hanya berada sebelum cutter; individual sheet hanya muncul setelah cutting; exact cutter architecture tetap ditandai unresolved.
