...
async function verifyPassword(password,account){
  const record=parsePasswordRecord(account?.password_hash);if(!record)return {ok:false,unsupported:false};
...
async function authStore(db){
  ...
  if(!account){
    ...
    // Generation 0 is guaranteed to still use the built-in initial password. Re-encode it
    // before verification so legacy 150k hashes never reach runtimes capped at 100k.
    const salt=crypto.randomUUID(),hash=await passwordRecord(DEFAULT_PASSWORD,salt);
