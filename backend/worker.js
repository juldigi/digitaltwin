...
async function passwordHash(password,salt){const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(password),'PBKDF2',false,['deriveBits']);return Array.from(new Uint8Array(await crypto.subtle.deriveBits({name:'PBKDF2',salt:new TextEncoder().encode(salt),iterations:150000,hash:'SHA-256'},key,256))).map(x=>x.toString(16).padStart(2,'0')).join('');}
async function authStore(db){await db.prepare('CREATE TABLE IF NOT EXISTS superadmin_auth (id INTEGER PRIMARY KEY CHECK(id = 1), salt TEXT NOT NULL, password_hash TEXT NOT NULL, generation INTEGER NOT NULL DEFAULT 0)').run();await db.prepare('CREATE TABLE IF NOT EXISTS superadmin_sessions (token_hash TEXT PRIMARY KEY, generation INTEGER NOT NULL, expires INTEGER NOT NULL)').run();await db.prepare('CREATE TABLE IF NOT EXISTS superadmin_attempts (client_hash TEXT PRIMARY KEY, count INTEGER NOT NULL, reset_at INTEGER NOT NULL)').run();let account=await db.prepare('SELECT salt,password_hash,generation FROM superadmin_auth WHERE id = 1').first();if(!account){const salt=crypto.randomUUID(),hash=await passwordHash(DEFAULT_PASSWORD,salt);await db.prepare('INSERT OR IGNORE INTO superadmin_auth(id,salt,password_hash,generation) VALUES(1,?,?,0)').bind(salt,hash).run();account=await db.prepare('SELECT salt,password_hash,generation FROM superadmin_auth WHERE id = 1').first();}return account;}
...
export default {async fetch(req,env){
  ...
  try{
    ...
    if(path==='/api/superadmin/login'&&req.method==='POST'){
      ...
      if(typeof data.password!=='string'||!await same(await passwordHash(data.password,account.salt),account.password_hash)){
    ...
    if(path==='/api/superadmin/password'&&req.method==='POST'){
      ...
      const data=await body(req),account=await authStore(env.DB);
      if(typeof data.currentPassword!=='string'||!await same(await passwordHash(data.currentPassword,account.salt),account.password_hash))return json({error:'Kata sandi lama salah.'},403,cors);
      ...
      const salt=crypto.randomUUID(),hash=await passwordHash(data.newPassword,salt);
