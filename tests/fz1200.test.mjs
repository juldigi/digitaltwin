...
test('FZ1200 keeps all three BMJ identities and supplier numbers remain reference-only',()=>{
 ...
 const ref=FZ1200_ASSET_PROFILES['BMJ-MCH-0007'];assert.equal(ref.uanchorFZ1200Reference.maxPileKg,1200);assert.deepEqual(ref.uanchorFZ1200Reference.openingM,[.76,1.64]);assert.equal(ref.uanchorFZ1200Reference.powerKw,9);
...
test('FZ1200 simulation clamps and lifts the same assembly before pivot-correct 180 degree turn',()=>{
 ...
 advanceTo(2.5);assert.equal(sim.state().clamped,true);assert.ok(upper.position.y<upper0-.40);assert.equal(sim.state().liftActive,true);
 advanceTo(5.9);const st=sim.state();assert.ok(st.turnAngleDeg>170);assert.ok(clamp.quaternion.angleTo(yoke.quaternion)<1e-9);assert.ok(clamp.position.y>sim.rest.clampPosition.y+.10);assert.ok(yoke.position.y>sim.rest.yokePosition.y+.10);
