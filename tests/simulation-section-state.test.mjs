...
test('simulation lifecycle preserves navigation until the user changes section',async()=>{
 ...
 setSimulation({active:false,running:false,stage:'Siap',progress:0});
 assert.equal(getState().activeSection,'simulation','ready panel stays in Simulasi');
 setSimulation({active:true,running:true,stage:'Feeder',progress:10});
 setSimulation({active:true,running:false,stage:'Feeder',progress:10});
 setSimulation({active:false,running:false,stage:'Siap',progress:0});
