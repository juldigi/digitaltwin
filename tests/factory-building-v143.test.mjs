...
test('V143 factory keeps DXF baseline while adding structural realism references',async()=>{
 ...
 assert.equal(meta.researchVersion,'V143');
 assert.equal(meta.buildingDetailPass,'V143_FACTORY_STRUCTURAL_AND_ARCHITECTURAL_REALISM');
 ...
 built.root.traverse(o=>{if(o.userData?.accuracy==='INDUSTRIAL_REALISM_REFERENCE_NOT_AS_BUILT')assert.equal(o.userData.researchVersion,'V143');});
});

test('V143 factory preserves open IPAL and prepared utility-routing boundary',async()=>{
 const [layout,fleet]=await Promise.all([loadActualPlantLayout(),loadFactoryFleet()]);
