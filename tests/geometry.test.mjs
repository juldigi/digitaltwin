...
test('V47 cutaway shows only functional roller bodies and relocates dampening to the OEM side',()=>{
 const t=new OffsetMachineTemplate();
 for(const id of ['press-0-ink-fountain-roller','press-0-ink-fountain-roller-body','press-0-damp-roller-18-body','dryer-uv-system'])assert.ok(t.findNode(id),`missing ${id}`);
 const journal=t.findNode('press-0-ink-roller-1-journals'),body=t.findNode('press-0-ink-roller-1-body');
 assert.equal(journal.userData.serviceDetail,true,'inking journal/lock should be service detail');
 t.setExteriorOpen(true);
 assert.equal(journal.visible,false,'journal/lock must not look like an extra roller in open exterior');
 assert.equal(body.visible,true,'functional inking roller body must stay visible in open exterior');
 assert.ok(t.root.userData.serviceDetailHiddenCount>0);
 const center=node=>{let mesh=null;node.traverse(o=>{if(!mesh&&o.isMesh&&o.geometry?.type==='CylinderGeometry')mesh=o;});const p=new THREE.Vector3();mesh.getWorldPosition(p);return p;};
 const plate=center(t.findNode('press-0-cylinder-plate-body'));
 for(const code of ['16','17','FR','19','18'])assert.ok(center(t.findNode(`press-0-damp-roller-${code}-body`)).x>plate.x,`dampening roller ${code} is not on the corrected OEM side of plate cylinder`);
 t.dispose();
});

test('V47 UV system is modeled inside dryer with lamps initially inactive',()=>{
 const t=new OffsetMachineTemplate(),uv=t.findNode('dryer-uv-system');
 assert.ok(uv);
 let lamps=0,beams=0;
 uv.traverse(o=>{if(o.userData.uvLamp)lamps+=o.userData.uvElementCount||1;if(o.userData.uvBeam){beams+=o.userData.uvElementCount||1;assert.equal(o.visible,false);}});
 assert.equal(lamps,3);assert.equal(beams,3);
 const dryerBox=new THREE.Box3().setFromObject(t.findNode('dryer-extension')),uvBox=new THREE.Box3().setFromObject(uv);
 assert.ok(uvBox.min.x>=dryerBox.min.x-.01&&uvBox.max.x<=dryerBox.max.x+.01,'UV cassettes extend outside dryer envelope');
 assert.ok(uvBox.min.z>=dryerBox.min.z-.01&&uvBox.max.z<=dryerBox.max.z+.01,'UV cassettes extend outside dryer side envelope');
 t.dispose();
});

test('V48 PU8 transition has no standalone tread and UV base is a machine support frame',()=>{
 const t=new OffsetMachineTemplate(),pu8=t.findNode('pu8-coater-access'),uvSupport=t.findNode('dryer-uv-support-frame'),stack=t.findNode('delivery-paper-stack');
 assert.ok(pu8&&uvSupport&&stack);
 let pu8Instanced=0,uvInstanced=0;
 pu8.traverse(o=>{if(o.isInstancedMesh)pu8Instanced++;});
 uvSupport.traverse(o=>{if(o.isInstancedMesh)uvInstanced++;});
 assert.equal(pu8Instanced,0,'PU8/coater transition still contains checker-plate tread geometry');
 assert.equal(uvInstanced,0,'UV support still uses checker-plate tread geometry');
 const supportBox=new THREE.Box3().setFromObject(uvSupport),dryerBox=new THREE.Box3().setFromObject(t.findNode('dryer-extension'));
 assert.ok(supportBox.min.y>1.0&&supportBox.max.y<1.30,'UV support frame should remain a low-profile equipment frame');
 assert.ok(supportBox.min.x>=dryerBox.min.x-.01&&supportBox.max.x<=dryerBox.max.x+.01,'UV support frame exits dryer envelope');
 t.dispose();
});
...
test('PU8 through delivery access geometry is supported and only meets adjacent module edges',()=>{
 ...
 for(const id of ['pu8-coater-access','coater-dryer-service-bay','dryer-delivery-access']){
  let cylinders=0;t.findNode(id).traverse(o=>{if(o.isMesh&&o.geometry?.type==='CylinderGeometry')cylinders++;});
  assert.equal(cylinders,0,`${id} still contains floating rail or roller geometry`);
 }
 t.dispose();
});
...
test('exterior enclosures physically contain upper PU and delivery mechanisms',()=>{
 const t=new OffsetMachineTemplate(),box=id=>new THREE.Box3().setFromObject(t.findNode(id));
 for(let i=0;i<8;i++){
  const enclosure=box(`press-${i}-ink`),train=box(`press-${i}-inking-train`),distribution=box(`press-${i}-inking-distribution`);
  for(const internal of [train,distribution]){
   assert.ok(internal.max.y<=enclosure.max.y+.001,`PU${i+1} upper roller exceeds ink enclosure`);
   assert.ok(internal.min.z>=enclosure.min.z-.001&&internal.max.z<=enclosure.max.z+.001,`PU${i+1} roller exits enclosure laterally`);
  }
 }
 const hood=box('delivery-hood');
 for(const id of ['delivery-chain-path','delivery-drive-sprockets','delivery-chain-tensioners']){
  const internal=box(id);assert.ok(internal.max.y<=hood.max.y+.001,`${id} exceeds delivery hood`);assert.ok(internal.min.z>=hood.min.z-.001&&internal.max.z<=hood.max.z+.001,`${id} exits delivery side enclosure`);
 }
 assert.ok(t.findNode('press-0-inking-train').children.length,'interior geometry must remain built');
 t.dispose();
});
