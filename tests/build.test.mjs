...
test('production HTML resolves the complete local module graph at a GitHub Pages subpath',()=>{
  ...
  assert.match(html,/<script type="module" src="\.\/src\/app\.js\?v=186"><\/script>/);
  const seen=new Set();
  function visit(file){
  ...
  assert.ok(seen.has(resolve(root,'src/universal-machine.js')));
  assert.ok(!seen.has(resolve(root,'src/machine-runtime.js')),'machine models load only when selected');
  assert.match(readFileSync(resolve(root,'src/engine.js'),'utf8'),/import\('\.\/machine-runtime\.js'\)/);
});
