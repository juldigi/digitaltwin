...
test('SHARK N650 preserves exact BMJ suffix without decoding installed options',()=>{
 ...
 assert.equal(SHARK_N650_SPEC.currentFamilyMaxSpeedMMin,400);assert.deepEqual(SHARK_N650_SPEC.currentFamilyMaxInspectionM,[.630,.300]);assert.deepEqual(SHARK_N650_SPEC.currentFamilyMinInspectionM,[.070,.090]);
 ...
 assert.equal(SHARK_N650_SPEC.olderFamilyReferences.older650Reference.inspectionSpeedMMin,220);assert.deepEqual(SHARK_N650_SPEC.olderFamilyReferences.older650Reference.maxSheetM,[.600,.450]);
 assert.ok(SHARK_N650_TECHNICAL_SOURCES.some(s=>s.authority==='primary'));assert.ok(SHARK_N650_TECHNICAL_SOURCES.some(s=>s.id==='SHARK-N650-EVIDENCE-BOUNDARY'));
});

test('SHARK geometry marks feeder camera reject and collection alternatives as references',()=>{
...
test('SHARK target blank must be inspected before reject tracking and bad-lane routing',()=>{
 const model=new SharkN650MachineTemplate(),sim=new SharkN650ProcessSimulation(model.root,model),b0=sim.blanks[0];sim.start();let now=1000;const advanceTo=t=>{while(sim.elapsed<t){now+=20;sim.update(now);}};
 advanceTo(.5);assert.equal(b0.result,null);assert.equal(b0.inspectedLap,-1);
 advanceTo(2.9);let state=sim.state();assert.equal(b0.result,'REJECT_DEMO');assert.ok(state.inspectedDemoCount>0);assert.equal(b0.mesh.position.z,0);
 advanceTo(5.55);state=sim.state();assert.equal(b0.result,'REJECT_DEMO');assert.equal(state.rejectTrackingActive,true);assert.equal(state.badRoutingActive,true);assert.equal(state.demoRejectActive,true);assert.ok(b0.mesh.position.z>0);assert.equal(state.demoRejectActuator,'PLATE_REFERENCE');assert.equal(state.demoRejectOnly,true);
 sim.dispose();model.dispose();
});

test('SHARK simultaneously supports tracked good and bad return lanes after inspection decisions',()=>{
 const model=new SharkN650MachineTemplate(),sim=new SharkN650ProcessSimulation(model.root,model);sim.start();let now=1000,seenScan=false,seenProcess=false,seenGood=false,seenBad=false;
 for(let i=0;i<480;i++){now+=20;sim.update(now);const s=sim.state();seenScan||=s.scanActive;seenProcess||=s.processingActive;seenGood||=s.goodRoutingActive;seenBad||=s.badRoutingActive;}
 const state=sim.state();assert.ok(seenScan&&seenProcess&&seenGood&&seenBad);assert.equal(state.suffixDecoded,false);assert.equal(state.installedCameraPackageVerified,false);assert.equal(state.installedRejectTypeVerified,false);assert.ok(state.inspectedDemoCount>0);sim.dispose();model.dispose();
});

test('SHARK tracked demo creates accepted and rejected collection while option claims stay bounded',()=>{
...
test('SHARK process stage sequence preserves inspection before routing',()=>{
